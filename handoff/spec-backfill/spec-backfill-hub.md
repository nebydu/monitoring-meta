# spec-backfill — hub 작업 spec: 통합본 호칭·경로 재배선

> **work-id**: `spec-backfill` / **대상**: `hub` / **발주일**: 2026-06-12
...
- v0.11은 내용 변경 릴리스(신규 결정 0): heartbeat protobuf 전환(ADR#2)·토픽 명명(ADR#5)·토픽 재명명(T4-1) 완료를 backfill. hub 코드 동작과 충돌 없음 — hub는 이미 `otlp_proto` 디코더·신 토픽명으로 구현돼 있다.
...
**대상(2026-06-11 spec-drift 보고 §2.1 실측 13곳 — 적용 시 `grep -rn "통합본"` 으로 재실측해 누락 보완)**:

| 파일 | 위치(당시 실측) |
|---|---|
| `AGENTS.md` | 48행 `통합본 v0.9` |
| `.claude/CLAUDE.md` | 14행 `통합본 v0.9를 방향 판단의 최상위 기준` / 18행 `통합본 v0.9`+경로 / 33행 |
| `.claude/agents/analyzer.md` | 3행 / 18행 |
| `.claude/agents/implementer.md` | 24행 `통합본 v0.9와 충돌하지 않아야` |
| `.claude/agents/reviewer.md` | 3행 / 16행 / 17행 (`§7.2 β 구조` 인용은 유지 — 절 번호 불변) |
| `.claude/agents/spec-guardian.md` | 40행 `통합본 v0.9 기준` |
| `.claude/agents/refactorer.md` | `v0.9` 매칭 행 |
