...
- [ ] 형제 repo 3건 컷오버 완료 보고 취합 후 **구 경로 stub(`docs/통합본_v0_9.md`) 제거**(meta 후속 — 별도 커밋).
- [ ] e2e baseline(60/0/0) 회귀 확인(컷오버 후, 각 handoff DoD).
- [ ] **기존 위상 불일치 — 별도 work-id로 분리(2026-06-12 codex-gate 지적 → 사람 결정: 분리)**: §7.5가 baseline 결정으로 서술한 항목을 §6.1.2·13장이 Open으로 유지 — Agentless polling 합류 토픽(단일 metrics-topic vs 별도, 13장 §H), 통보 정책 평가 엔진, Knox 조직도 동기화 주기, 시스템 자체 OpenSearch 분리, WebSocket 권한 필터링(13장 §B/§C/§E/§G 성격). **v0.8부터 존재, v0.11 diff 무접촉(기계 대조 — 29 hunk 중 해당 절 0)**. 항목별 사람 결정이 필요한 위상 정리이므로 v0.11 "신규 결정 0" 원칙에 따라 범위 제외 — 후속 work-id(가칭 `open-alignment`)에서 §7.5↔13장 항목별 대조·정리.
