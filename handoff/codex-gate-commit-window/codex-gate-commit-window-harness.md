...
1. 상태 파일에 `verified_commit`(vc) 필드 — plugin은 상태가 plugin data dir에 있으므로 그 레이아웃 기준으로. ~~`~/.claude/plugins/data/harness-monitoring/<repo>/`~~ **표기 정정(실행 결과 §7)**: 실제 `${CLAUDE_PLUGIN_DATA}`는 repo 미구분 flat 공유 디렉터리였음 — vc 오염 방지를 위해 core가 `<data>/<repo명>/` 하위 분리를 직접 구현(5c1bff1).
...
## 7. 결과 기록 (meta 세션 검증, 2026-06-11)

- **완료** — harness commit `37ad999`(검증 윈도우 도입) + `5c1bff1`(plugin 공유 data dir을 repo별 하위 디렉터리로 분리), 작업 트리 clean. `shared/hooks/README.md`에 검증 윈도우 절 추가.
- 검증: 시나리오 12종 62 assert 전부 통과(커밋 후 클린 트리 트리거 = 사각지대 해소 증명 포함). plugin 캐시 user+project(hub/script-agent/infra) 전 scope `5c1bff13d480` 갱신·소스 일치 — meta에서 캐시 디렉터리 존재 재확인. hub 실제 Stop 1회로 bootstrap-origin SKIP + vc 전진 + repo별 state 기록 확인.
- **실행 중 발견 2건**:
  1. spec §3의 data dir 표기(`<repo>/` 구분)가 실제와 달랐음 — `${CLAUDE_PLUGIN_DATA}`는 flat 공유라 로그에 hub·infra가 혼재(실증). core가 `<data>/<repo명>/` 분리를 구현해 vc 오염 차단(§3 표기 정정 완료).
  2. 구 core의 escalation 임계가 '초과 시'(-gt)여서 reference(meta hook, '도달 시' -ge)보다 1회 늦게 발동 — 동일하게 통일.
- 참고 관찰: 더티 pass 후 동일 패치 커밋은 gate_key 캐시 적중으로 중복 재검증 없이 skip(비전진, 다음 변경에서 전체 윈도우 재검증) — record 문서 §5 트레이드오프의 실제 양상이 예상보다 유리함.
- 잔여 운영 절차: 실행 중이던 Claude 세션은 재시작해야 새 캐시 적용.
