...
| `minio/init-buckets.sh:2`, `opensearch/init-templates.sh:2` (같은 패턴 **2개 파일 = 2곳**) | `운영 골격(P-2)` | `운영 골격 슬라이스` |

> 위치 수 세는 법: docker-compose.yml 6곳 + init 스크립트 2곳 = **총 8곳**(표 행은 7개 — 마지막 행이 파일 2개를 묶음).
...
## 5. 완료 기준
- [ ] §3 표의 8곳 주석 치환 — 파일 3개: `docker-compose.yml`(6곳)·`minio/init-buckets.sh`(1곳)·`opensearch/init-templates.sh`(1곳) (diff가 주석 라인만 포함)
...
```json
{
  ...
  "outputs": ["docker-compose.yml", "minio/init-buckets.sh", "opensearch/init-templates.sh"],
  "findings": ["치환 8곳 결과", "잔존 grep sweep 0건 여부", "docker compose config 무오류 여부", "diff가 주석 라인만 포함하는지"],
  "blockers": ["치환표와 실제 파일 내용이 불일치하는 곳(있으면)"],
  "next_action": "한국어 메시지로 커밋 후 monitoring-meta 세션에 결과 JSON 전달"
}
```

## 7. 결과 기록 (meta 세션 검증, 2026-06-11)

- **완료** — infra commit `fc4143c` "docs: 폐지 ID(G/P/M) 주석 표기 갱신 — 주석 8라인(치환표 7행) (id-cleanup)", 작업 트리 clean.
- meta에서 읽기 전용 재검증: `docker-compose.yml`·`minio/`·`opensearch/`에 폐지 ID(`G-[1-5]|P-[1-4]|M-1`) **0건**.
- 잔존 매치는 `docs/decisions/proposal-review-*.json` 2건뿐 — §3 out-of-scope(스냅샷 무수정)에 해당, 정상.
