...
- 기준 문서(`docs/envelope.md` §4.2, 통합본 §6.8.1 예외문): heartbeats-topic은 OTLP 위임군으로 envelope 4종 예외. **이 성격은 protobuf 전환 후에도 불변.**
...
- 통합본/기준 문서 논리명 = **`heartbeats-topic`** (통합본 §4.4.1, envelope.md §4.2, kafka-payloads.md).
...
### 2.2 기준 문서가 이미 시사하는 방향 (단, 강한 결정은 아님)

- kafka-payloads.md는 **"OTLP MetricsData protobuf"** + **"OTel Java SDK 파싱"** + **"shared-libs/otel wrapper"** 를 명시한다. 즉 기준 문서는 **커스텀 proto가 아니라 OTLP 표준 protobuf**를 시사하고, hub의 수동 JSON 파싱 → OTel SDK 디코더 전환을 시사한다.
...
**후보 A-1 — OTLP 표준 MetricsData protobuf 그대로 (기준 문서 시사안)**
- Collector `encoding: otlp_proto`로만 바꾸면 wire는 OTLP 표준 `ExportMetricsServiceRequest`/`MetricsData` protobuf.
- 장점: proto 스키마를 **우리가 정의·관리하지 않는다**(OpenTelemetry proto가 곧 스키마). Agent SDK·Collector·hub SDK가 모두 같은 표준을 공유. metrics-topic(Phase 2)과 일관. kafka-payloads.md "OTLP MetricsData protobuf" 서술과 일치.
...
- 단점: **OTel 파이프라인을 벗어난다.** Collector 표준 exporter로는 못 만들고, Agent가 Kafka에 직접 producer를 갖거나 Collector custom processor가 필요 → 데모의 "Agent는 Kafka heartbeats 직접 안 다룸" 구조를 깸. 통합본 §6.7(OTel SDK push) / kafka-payloads(OTLP MetricsData) 방향과 충돌 소지. **기준 문서 위배 가능성** → 채택 시 통합본 영향 검토 필요(§9).

**후보 A-3 — OTLP protobuf + Schema Registry 등록**
- A-1에 더해 Apicurio 등 SR에 OTLP proto 스키마 등록.
- 장점: Phase 2/3 SR 도입(ADR #1)과 선제 일치.
...
> **분석자 관점(결정 아님)**: 기준 문서 서술·데모 구조 보존·ADR #1 일치를 종합하면 **A-1이 기준 문서와 가장 마찰이 적다.** 단 최종 채택은 사람 결정 사안.
...
- 장점: meta가 proto 파일을 소유·배포할 필요 없음. 가장 가벼움. kafka-payloads "shared-libs/otel = wrapper 제공"과 일치(스키마가 아니라 파싱 wrapper만 공유).
- 단점: 버전 skew 위험(OTel proto 버전 불일치). 통제는 라이브러리 버전 핀으로.

**후보 B-2 — meta 기준 문서에 .proto 사본 보관 + 각 repo가 참조/생성**
- OTLP proto를 meta가 vendoring하고 양쪽이 protoc 코드 생성.
- 장점: 단일 기준 문서. 단점: OTLP 표준 proto를 재배포·재생성하는 중복. A-1에서는 과잉.

**후보 B-3 — 공유 repo / shared-libs 모듈 신설**
- kafka-payloads.md가 언급한 `shared-libs/otel`을 실제 모듈로 만들어 hub 측 OTLP 파싱 wrapper 제공.
- 장점: hub 내 heartbeat/metrics(Phase 2) 파싱 일원화. 단점: 모듈 신설 비용. **`shared-libs/otel`의 실체(언어·배포·소유)는 기준 문서 미정의** → §9 blocker.

> envelope.md §7과 동일 논리: envelope(헤더 markdown spec)과 payload 직렬화(IDL)는 층위가 다르다. heartbeat은 payload 직렬화 층이며 OTLP 표준에 위임하므로, "공유"의 실체는 **proto 파일 공유보다 OTel 라이브러리 버전 일치 + (hub 측) 파싱 wrapper**에 가깝다.
...
- `internal/heartbeat/heartbeat.go`: **변경 불필요 가능성 큼**(SDK가 OTLP HTTP로 push, wire format은 Collector 소관). OTLP Go SDK 버전 일치만 확인.
...
### 7.4 meta (문서)
- 토픽명 정정(§1.4)·`shared-libs/otel` 실체 확정 시 kafka-payloads.md / envelope.md 일치 갱신은 **별도 제안서(handoff/<work-id>/통합본-update-proposal)** 경로. 본 분석에서는 수정하지 않음.
...
1. **[wire schema 결정]** A-1(OTLP 표준 protobuf) vs A-2(커스텀 proto) vs A-3(+SR). 기준 문서는 A-1을 시사하나 ADR #2 결정란은 비어 있음. **A-2 채택 시 데모의 "Agent는 Kafka heartbeats 직접 안 다룸" 구조 및 통합본 §6.7/kafka-payloads 방향과 충돌 가능** → 통합본 영향 검토 필요.
2. **[공유·관리 방식]** B-1/B-2/B-3. 특히 kafka-payloads.md가 언급한 **`shared-libs/otel`의 실체(언어·배포·소유 주체)가 기준 문서에 미정의** — 추측 금지.
...
6. **[토픽명 정정 묶음 여부]** `heartbeats`(데모) → `heartbeats-topic`(기준 문서) 정정은 데모 정정 11개 항목(통합본 §6.9.2)의 별개 사안. ADR #2와 함께 처리할지 분리할지 결정 필요.
...
{
  "status": "blocked",
  "outputs": ["handoff/adr-002/adr-002-analysis.md"],
  "findings": [
    ...
    "기준 문서(kafka-payloads.md)은 'OTLP MetricsData protobuf + OTel Java SDK 파싱 + shared-libs/otel wrapper'로 A-1 방향을 시사하나, ADR #2 결정란(스키마·공유·컷오버)은 비어 있다.",
    "데모 토픽명 'heartbeats'와 기준 문서 논리명 'heartbeats-topic' 불일치는 데모 정정 별개 항목이며 ADR #2와 직접 묶이지 않는다.",
  ...
  "blockers": [
    ...
    "shared-libs/otel 실체(언어/배포/소유)가 기준 문서 미정의",
  ...
  "next_action": "사람이 §9의 6개 결정 사안(특히 wire schema A-1/A-2/A-3, 컷오버 C-1/C-2/C-3)을 입력하면 analyzer가 ADR #2 결정란을 채우고 handoff/adr-002/adr-002-hub.md, handoff/adr-002/adr-002-script-agent.md(필요 시 adr-002-infra) 분배 파일을 작성한다."
}
```
