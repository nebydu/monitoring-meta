...
- 작성 일시: 2026-05-27
- 정본 기준: `docs/통합본_v0_9.md`(Phase 1+ 목표 spec), `docs/kafka-payloads.md`(별첨 payload spec)
...
- **1라운드: `docs/envelope.md` 미확정. 본 문서는 후보안 분석만이며 정본 envelope spec을 확정하지 않는다.**
- 위상 원칙: 데모 spec v0.2.1 = "Phase 0 코드가 회귀 없이 지켜야 할 동작 spec". 통합본 v0.9 = "Phase 1+ 도달 목표 spec". 둘을 같은 ground truth로 다루지 않는다.
...
- 양쪽 모두 `x-trace-id`는 값이 빈 문자열/null이면 헤더 자체를 생략 (script-agent `kafka/envelope.go` L25, hub `CommandPublisher` L71) → 데모 spec §2.2 "○(선택)" 동작과 정합.

> **위상 메모.** 위 표의 "필수/선택"과 "데모 동작"은 Phase 0 회귀 기준이다. Phase 1+ 목표(중복 검사 도입, trace 복원, source 값 확장)는 통합본 결정이며 데모 코드가 아직 구현하지 않는다.
...
- 후보 A: payload `trace_id` 제거, 헤더만 정본 → consumer가 chain 위해 헤더를 payload로 복사해야 함(불편).
- 후보 B: 둘 다 유지 + "헤더가 transport 정본, payload는 동일 값의 도메인 사본"으로 envelope spec에 관계 명문화.
...
- 판정: 경계 명확. envelope = 전송 메타데이터 4종 한정, 도메인 식별자는 payload/키. 추가 결정 불필요하나 **envelope spec에 "도메인 ID는 envelope에 넣지 않는다"는 경계 원칙을 명문화**할지가 B-4 쟁점.
- 후보 A: 원칙만 산문으로 서술.
...
- **회귀 0 보장 조건**: 헤더 키 4종 문자열·`x-message-version` 값 `"1"`·source 값(데모 3종)·옵셔널 trace 생략 로직 — 양쪽 repo 현 구현이 이미 충족(§1 코드 정합).
...
> **위상 결론.** envelope 헤더 4종은 데모↔v0.9가 **포맷 수준 완전 하위호환**이다. 정정은 전부 "consumer 측 동작 추가" 또는 "예외 토픽 직렬화"이며, Phase 0 발행 코드는 회귀 없이 통과한다.

---

## 5. shared-libs/envelope 모듈 위상
...
| 전략 | 방식 | 장점 | 단점 |
|---|---|---|---|
| (1) 수동 동기화(현행) | envelope.md 정본 + 각 repo가 spec 보고 구현 | 추가 인프라 0, Phase 0 동작 중 | drift 위험(spec-sync로만 검출), 언어별 2벌 유지 |
| (2) 코드 생성 | 단일 정의(YAML/JSON) → Java/Go 코드 생성 | 단일 정본에서 양 언어 동기 | 빌드 파이프라인·생성기 도입 비용, polyrepo에 생성물 배포 경로 필요 |
...
- **추천: Phase 1은 (1) 수동 동기화 유지 + spec-sync drift 검사로 통제, "shared-libs/envelope"는 "정본 envelope.md + 언어별 thin 구현 가이드"로 정의.** 코드 생성(2)은 발행자 수가 늘어 drift 비용이 커지는 Phase 2 재고 카드로. (사람 결정 필요 — §7)
...
- **envelope spec 작성 원칙(2라운드 반영용)**: "envelope은 transport-layer 헤더 규약이며 payload 직렬화 형식(JSON/Avro/Protobuf)과 독립적으로 정의된다"를 명문화. SR 도입은 payload 스키마 관리 경로이지 envelope 구조 변경이 아님.
...
### Q1 (B-1). `x-message-version` 정책 — **확정: 후보 A 채택 (+ major-only 정합)**
...
- 보충: 1라운드 표현 "정수 단조 증가"는 codex-gate 검증 과정에서 kafka-payloads 변경 정책과 정합하도록 **"major-only"로 구체화**했다(코드 회귀 0, 통합본 8.2 "단순 정수" 정합).
...
- 채택: 헤더 `x-trace-id`(transport 정본) + payload `trace_id`(동일 값 도메인 사본) 둘 다 유지 + "항상 동일해야 한다" 불변식 명시.
- 반영처: envelope.md §5.

### Q4 (B-4). envelope 경계 명문화 — **확정: 후보 B 채택**
- 채택: 원칙 산문 서술 + negative list("envelope에 넣지 않는 것") 병기.
- 반영처: envelope.md §3.

### Q5. shared-libs/envelope 모듈 실체 — **확정: 후보 (1) 채택**
- 채택: Phase 1 수동 동기화 + spec-sync 통제. "정본 envelope.md + 언어별 thin 구현 가이드" 모델. 코드 생성/IDL 통합은 Phase 2+ 후보로만 표기.
- 반영처: envelope.md §7.
...
> 위 항목은 envelope.md 정본에 **결정으로 쓰지 않고**, "Phase에 따라 결정 예정 / 13_open 참조"로 표기해야 한다.
...
- `script-agent/internal/kafka/envelope.go` `BuildHeaders` — 헤더 생성 + 옵셔널 trace 생략 로직. envelope 모듈화(Q5) 시 정본 가이드 대상.
...
- ADR #6 (메시지 키) — §3 키 정책 정합(결정됨).

### 9.4 후속 작업
1. 사람이 §7 Q1~Q5 결정 입력.
2. 2라운드: `docs/envelope.md` 정본 작성(본 분석 §1~§3 + 결정 반영, §8 Open은 보존 표기).
3. `spec-sync` drift 검사 → 데모 spec §2.2 ↔ envelope.md ↔ 양쪽 repo 구현 정합 확인.
4. 세션 Stop 시 `codex-gate`가 통합본 6.8/6.9 ↔ envelope.md 정합성 read-only 검증.
