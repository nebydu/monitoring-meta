# 작업 spec — handoff-restructure (script-agent)
...
| 대상 repo | `script-agent` | 문서 표기만 수정, 코드 변경 없음 |
...
`monitoring-meta/handoff/<work-id>/<work-id>-script-agent.md`다(규칙 전문: `../monitoring-meta/handoff/README.md`).
script-agent 문서들이 구 평면 패턴 `handoff/<work-id>-script-agent.md`를 표기하고 있어 새 규약으로 갱신한다.

## 3. 작업 범위

### 해야 할 것 — 패턴 문자열 치환 (한 종류)

`monitoring-meta/handoff/<work-id>-script-agent.md` → `monitoring-meta/handoff/<work-id>/<work-id>-script-agent.md`

대상 파일(2026-06-10 기준 위치 힌트 — 실제는 패턴 grep으로 전수 확인):
- `.claude/CLAUDE.md` (약 :19, :27, :30)
- `.claude/agents/analyzer.md` (약 :3, :12, :27)
...
## 8. 완료 기준 / 검증
- [ ] `grep -rn 'handoff/<work-id>-script-agent' .` 결과 0건 (전부 `handoff/<work-id>/<work-id>-script-agent`로 치환)
- [ ] 코드·테스트 영향 없음 (`go test ./...` 불필요 — 문서만)
