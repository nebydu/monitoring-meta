...
| 선행 | phase1-001(범위 확정), envelope.md(기준 문서) |
...
- envelope.md §7이 현재 구현 위치를 `internal/model/envelope.go`(키 상수+고정값), `internal/kafka/envelope.go`(`BuildHeaders`, 옵셔널 trace 생략)로 명시한다 — 이미 기준 문서와 일치.

## 3. ground truth 참조 (상대 경로, 사본 두지 마라)

- `../monitoring-meta/docs/envelope.md` — envelope 4종 기준 문서. 특히 **§2.2(헤더 정의)·§2.3(알 수 없는 x-source 가드)·§4.1(공통 토픽군)·§6(Phase 0 회귀)·§7(현재 구현 위치)**
...
- **producer 2종 — 이미 envelope 4종 발행 완료**:
  ...
  - 공통 빌더 `internal/kafka/envelope.go` `BuildHeaders`가 `x-message-id`/`x-message-version="1"`/`x-source="script-agent"` 발행, `traceID==""`면 `x-trace-id` 생략 — envelope §2.4·§6 일치. → **producer 신규 적용 작업 없음. 회귀만 보장.**
...
| `x-trace-id` 발행 값 채우기 / 복원 | 현재 `""` 발행은 일치(옵셔널 생략). 값 주입은 OTel 통합 별도 작업 | envelope §2.4, 별도 |
| 토픽 **재명명**(`commands`→`command-topic` 등) | envelope은 토픽명 독립, D-4(1) 승인 대기 | Track 4-1 |
...
- D-4(1) 토픽 명명 컨벤션은 `adr/0005` 승인 대기지만 **본 작업과 독립**(토픽명 미변경). 추측으로 토픽명을 고정하지 마라.
