...
> **commit pin 주의**: 위 pin은 phase0-cleanup 정리 커밋(데모 spec 단일 정본화 + ROADMAP/envelope/PROJECT_OVERVIEW/.claude repoint + HANDOFF.md archive 강등)에서 채워졌다. **실행 직전 `git rev-parse HEAD`(monitoring-meta)로 재확인**하고 hub repoint 경로가 그 commit의 phase0-snapshot 정본을 가리키는지 확인한다.
...
monitoring-meta 단일 정본으로 통합한다. retire 불가(데모 spec 회귀 계약 중 e2e는
§5.4만 커버) → phase0-snapshot 동결 baseline 보존. hub repo의 사본은 삭제하되,
사본을 가리키던 모든 참조를 먼저 새 정본 경로로 재배선한다.
...
루트 HANDOFF.md가 `archive/`로 강등(메타 메인 세션 처리)되므로 hub 쪽 HANDOFF 인용을 정리한다.

- `hub/.claude/CLAUDE.md`, `hub/.claude/agents/analyzer.md`에서:
  - **"HANDOFF.md §5 체크박스를 수동 갱신한다" 류 운영 룰을 제거**한다(강등 후 무효).
  - HANDOFF.md를 provenance(출처)로 인용한 문장은 **인라인화**(필요한 사실을 그 문서에
    직접 적음)하거나 **제거**한다. 외부 dangling 인용으로 남기지 않는다.
- 단, 루트 HANDOFF.md 파일 자체의 강등/이동은 **메타 메인 세션 몫**이다. hub세션은 hub repo
  안의 인용만 정리한다.
...
2. (검증) hub repo 내 데모 spec **LIVE dangling 0**:
   `rg "docs/monitoring-demo-message-spec-v0\.2\.1\.md" hub` 결과 중 *현행 유효 참조*가
   새 경로(`../monitoring-meta/docs/phase0-snapshot/...`)만 남고 구 로컬 경로는 0건.
...
- 루트 HANDOFF.md archive 강등 **시점**과 본 repoint 시점의 순서는 메타 메인이 조율
  (분석서 D-C2). 강등이 hub repoint보다 먼저면 hub 내 HANDOFF 인용이 일제히 dangling되니,
  4.3을 그 전제로 수행한다.
...
작업 종료 시 아래를 반환한다:
```json
{"status":"ok|blocked|failed","outputs":["수정/삭제 파일"],"findings":["dangling 검증 결과"],"blockers":["HANDOFF 강등 순서 등"],"next_action":"한 줄"}
```
