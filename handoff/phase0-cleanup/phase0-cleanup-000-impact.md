...
>
> **문서 성격 (혼동 금지)**: 데모 spec v0.2.1 = Phase 0 회귀 ground truth.
...
두었던 **데모 spec v0.2.1을 monitoring-meta 단일 기준 문서**
(`docs/phase0-snapshot/monitoring-demo-message-spec-v0.2.1.md`)으로 통합하고,
이를 가리키던 모든 참조를 새 경로로 재배선한다. **삭제가 아니라 통합**이다
(데모 spec은 phase0-snapshot에 고정 baseline으로 보존).

데모 spec 참조 35파일 + HANDOFF.md 참조 10파일 = 총 45개 참조 surface를 본 표에 집약한다.

---

## 1. retire 조건 판정 (데모 spec 고정 baseline 보존 근거)

**판정: retire(폐기) 불가 → phase0-snapshot 고정 baseline으로 보존.**
...
- 즉 데모 spec이 보장하는 회귀 계약의 일부만 자동 검증되므로, spec 문서 자체를
  ground truth 고정 baseline으로 유지해야 회귀 기준이 사라지지 않는다.
- 따라서 데모 spec은 `docs/phase0-snapshot/`에 단일 기준 문서로 보존하고, 각 repo
...
> (3) 이들 참조의 의미는 "그 작업 시점에 데모 spec 기준 문서/사본이 hub/docs·script-agent/docs에
> 있었다"는 **당시 사실**이며, 경로는 *당시 기준*으로 해석해야 한다(현재 기준 문서는
...
| 파일 | 처리주체 | 비고 |
|---|---|---|
| 루트 `HANDOFF.md` | meta메인 | `archive/`로 격하(사용자 확정 2). analyzer/sub-agent 비수정 대상 |
| `.claude/*` (meta repo) | meta메인 | 본 sub-agent Write 범위 밖 |
| 각 repo `.claude/*` 중 HANDOFF.md 격하 연동 후속 | meta메인 ↔ repo세션 | HANDOFF archive 후 깨지는 참조는 repo세션 handoff에 포함(§3) |
...
- **D-C1 (ROADMAP HANDOFF source_ref 재배선)**: ROADMAP_PHASE1_v0_3.md는
  `HANDOFF.md §5(작업 상태)` / `§7(미결정)`을 **의도적 source_ref 표기 규칙**으로 채택했다
  ...
  를 ROADMAP 자체 절로 **무손실 치환 불가**. 게다가 루트 HANDOFF.md는 `archive/` 격하이
  **메인 세션 처리 대상**이라, 격하 후 (a) archive 경로로 재배선할지 (b) ROADMAP §4·§5에 §5/§7
  요지를 인라인 흡수할지 (c) 참조를 통합본/envelope 기준 문서로 대체할지 결정이 선행돼야 한다.
  → analyzer는 ROADMAP를 변경하지 않았음. **[해소 2026-06-05] D-C1 결정 = ROADMAP 자체 절+통합본 기준 문서 재배선((b)/(c) 혼합). 메인 세션이 §5/§7 source_ref 15곳을 부분 Edit로 재배선(be990c7). §4·§16 "HANDOFF"(handoff/ 역할 명칭)는 보존.**
...
- **D-C2 (HANDOFF.md archive 연동 repo 참조 순서)**: 각 repo `.claude/CLAUDE.md`·
  ...
  세션 handoff에 포함했으나, 루트 HANDOFF.md 격하 자체는 메인 세션 몫이다. **격하 시점**과
  repo repoint 시점의 순서를 메인이 조율해야 한다(격하 먼저면 repo의 HANDOFF 인용이 일제히 dangling). **[해소 2026-06-05] D-C2 기본값 적용: 메인이 meta-side 의존을 먼저 끊고 be990c7에서 archive 격하 → repo는 분배 세션에서 자기 인용 정리. 격하~repoint 사이 dangling 창을 줄이려 루트에 redirect tombstone `HANDOFF.md`(전체 본문은 `archive/HANDOFF.md`)를 둠.**

- **(없음 확인)** 통합본 v0.9 `[Open]` / `13_open.md` / 미결 ADR을 새로 건드리는 변경은 본 작업에
  포함되지 않음. PROJECT_OVERVIEW의 Alert/Incident phase 소속 충돌은 **기준 문서(통합본/ROADMAP)이 이미
...
- 문서 버전: **v0.3 (기준 문서 후보) 보존**.
- 결론: **(정정)** analyzer 작성 시점엔 ROADMAP 무변경이었으나, 사용자 D-C1 결정 후 메인 세션이 루트 `HANDOFF.md §5/§7` source_ref 15곳을 부분 재배선(be990c7). §4·§16 "HANDOFF"(handoff/ 역할 명칭)는 보존. 위 4개 무결성 지표(섹션/ADR/pin/버전)는 **재배선 후에도 그대로 유지됨**을 재확인.
