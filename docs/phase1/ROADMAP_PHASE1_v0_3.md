...
>
> **결정 (push — 사람 입력)**
> - **D-2** β/γ 모듈 분리 — 최고 레버리지(DoD-4 게이트). **타이밍: §C 협의 입력 8개 수집은 지금 요청, β/γ 결정은 T2-1 게이트까지 유보**(`handoff/decisions/d2-escalation-timing.md`). 통합본 §C Open — 추측 금지.
...
> **성격**: 이 문서는 `monitoring-meta/docs/통합본_v0_9.md`를 최상위 기준으로 삼는 **Phase 1 기준 문서 후보 ROADMAP**이다. 입력 draft(`ROADMAP_PHASE1_draft_v0_2.md`)를 Pass 1 normalization(`handoff/phase1-000/phase1-000-roadmap-normalization.md`) 검증 결과로 정정·반영해 생성했다. draft를 그대로 기준 문서화한 것이 아니다.
>
...
| Pass 1 검증 | `handoff/phase1-000/phase1-000-roadmap-normalization.md` |
...
> **항목 11 command-topic routing 사람 승인 (2026-06-05)**: 통합본 §8.3 ADR#4 결정 컬럼이 "동일 + zone 단위 토픽 routing"을 직접 포함하므로 ADR#4 확정 소속으로 승인한다. D-5로 되돌리지 않음(추측 아닌 §8.3 직접 도출). (Pass 1 `handoff/phase1-000/phase1-000` line 122는 "ADR#4 간접 소속 후보 [결정 필요]"로 두었으나, §8.3 결정 컬럼이 zone routing을 명문으로 담고 있어 사실 소속으로 확정한다.)
...
| T1-5 | 사이트별 운영 정보 정리 | `통합본 §13_open §A` | monitoring-meta, infra | IN_PROGRESS | topology/security/node sizing | 외부 정보(G-3) | `handoff/phase1-014/phase1-014-site-ops-inputs.md` | site별 운영정보 matrix |
...
| 5 | `handoff/phase1-012/phase1-012-module-split-decision.md` | β/γ 및 deployment map 결정 (D-2) | monitoring-meta, hub, infra |
