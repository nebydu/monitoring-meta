...
>
> **진행 중 — 형제 repo 발주됨**
> - **alert/notification 토픽 추가 + 신규 토픽 키 정의**(T2-4 ∪ T4-4 합본, work-id `topics-and-keys`, 발주 2026-06-25) — hub(토픽 상수 2개 + producer 골격, 작업 깊이 (b))·infra(kafka-init 5→7토픽) 발주. 키 wire 인코딩 `alert-key:v1:{rule_id}:{target_id}`(null이면 `agent-offline`)·notification=`incident_id` 확정, x-source 필수 주입. proposal-review approve. 복귀 게이트=e2e 회귀+계약 일관성+kafka-payloads wire 인코딩 반영(T4-4 DONE 선결). script-agent 영향 0. → §11/§13, `handoff/topics-and-keys/`
>
> **임계 경로 후속**
...
| T4-4 | 메시지 키 토픽별 정의 (ADR#6 잔여: 신규 토픽 key) | `통합본 §8.3 ADR#6`, `통합본 6.8.2`, `통합본 §6.9.5` | hub, script-agent, monitoring-meta | **IN_PROGRESS (발주 2026-06-25)** | ordering/partitioning semantics | topic naming decision(T4-1, DONE) | `handoff/topics-and-keys/` (**T2-4와 합본**, 000-impact + hub + infra) | topic별 key rule + test (기존 토픽 키 기구현 — §5.1 #6 / 신규: alert=`(rule_id,target_id)`·notification=`incident_id`는 §6.9.5 확정). **발주 시 신규 확정(meta 결정 2026-06-25)**: alert key **wire 인코딩** `alert-key:v1:{rule_id}:{target_id}`(null이면 `agent-offline`), notification=`incident_id` 원문. x-source(envelope 필수)=publisher 필수 주입(placeholder 금지). proposal-review approve. **복귀 게이트=kafka-payloads `alert-topic` 키 절에 wire 인코딩 반영(T4-4 DONE 선결)**. script-agent 작업분 0(신규 토픽 키 한정). |
