...
| T0-3 | envelope 적용 handoff 생성 | `ROADMAP §9 Track 0` | monitoring-meta | TODO | hub/script-agent 후속 작업 | T0-1, T0-2 (둘 다 해소) | `handoff/phase1-002-envelope-remaining-topics.md` | repo별 수정 범위와 테스트 명시 |
...
> **D-9 해소 (2026-06-03)**: `alert-topic`/`notification-topic` 등 topic set 확장이 Phase 1 확정인지 ADR#5 종속 Open인지의 결정은 **RESOLVED**다 — alert/notification 토픽은 **Phase 1 확정**이다(통합본 §6.9.3 (다) Phase 1 표가 두 토픽을 "Kafka 토픽 추가"로 명시 + §6.9.5 토픽표가 "(다) v0.7 신규 (Phase 1)"로 확정, key rule까지 박음 — 확인된 사실). 명명 규칙(zone 단위+의미 기반 prefix)은 **D-4(1)**(`adr/0005`)로, 토픽 재구조 실행 선후는 **D-4(2)**(RESOLVED 2026-06-04)로 남는다. result-topic 분리 자체도 §6.9.2 항목1로 Phase 1 확정. (결정: human, 2026-06-03 (A) Phase 1 확정.)
...
> **현재 상태 요약(v0.3)**: D-9·D-4(2) 해소, D-4(1) `adr/0005`로 승격(비준 대기), 8토픽 계약 매트릭스(§7) 계약렌즈 정리, command-topic routing ADR#4 확정 귀속 비준(2026-06-05). 상세 gate-round 이력(gate-1~9 / round-2 CRITICAL·sweep·SPEC / D-9·D-4 반영 매핑)은 `handoff/phase1-000b-consistency-checklist.md` 부록 참조.
...
| **v0.3 (정본 후보, 현행)** | Pass 1 normalization + codex-gate 다회차 정정 + D-9·D-4 사람 결정 반영. **현재 상태**: D-9·D-4(2) RESOLVED, D-4(1) `adr/0005` 비준 대기, §7 8토픽 계약렌즈 정리, command-topic routing ADR#4 확정 귀속 비준(2026-06-05). 잔여 미결 = §17 D-1·D-2·D-4(1)·D-5·D-8(+ 통합본 Open §A/§C/§J). §1 기준 commit pin `4940e1a`·버전 v0.3 유지. 상세 차수별 정정 이력은 부록(`handoff/phase1-000b-consistency-checklist.md`)으로 이관. |
...
> **요약**: Pass 1 normalization(N-1~N-5 / C-1~C-4 / S-1~S-4) + codex-gate 다회차 정정(1차 9건 / 2차 round-2 5건 / 3차 D-9 반영 / 4차 §7 계약렌즈·D-4·DoD-1·§16 위상) + D-4 사람 결정(2026-06-04) + command-topic routing 귀속 비준(2026-06-05)을 v0.3에 모두 적용 완료.
>
...
>
> **현재 잔여 결정 필요**(회고가 아닌 현 상태): §17 D-1·D-2·D-4(1)·D-5·D-8. (D-4(2)·D-9는 RESOLVED, D-4(1)은 `adr/0005` 비준 대기.) 통합본 미결 Open(§A/§C/§J) 및 정정 귀속(D-5)·구체 명명 컨벤션(D-4(1))은 추측 금지 대상으로 보존한다(토픽 최종 이름 동결 금지). §1 기준 commit pin(`4940e1a`)·버전 v0.3은 유지한다.
