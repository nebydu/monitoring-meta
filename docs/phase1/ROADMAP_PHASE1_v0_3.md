...
>
> **발주 완료 — 형제 repo 구현 대기**
> - **result-topic 분리**(T4-2) — **handoff 발주 완료(2026-06-13, `handoff/phase1-041/`)**. §5 결정 확정(payload 토픽만 분리·동시 컷오버·env 2키·회귀 R-A+R-B+R-C). infra(1순위)→hub+script-agent(동시) 구현 → meta 복귀 게이트(e2e R-A+R-B+R-C + 계약 문서 5종 갱신 + T4-2/T4-5 DONE). → §13
> - **계약 문서 갱신**(T4-5) — **1차 정합화 완료(2026-06-13, IN_PROGRESS), `handoff/phase1-044/`**. 2차 갱신(현행 물리명 분리 반영)은 T4-2 구현 후 meta 복귀 게이트에서 → DONE. → §13
...
| #19 | result payload 정렬 단계화 | 토픽 분리(T4-2)와 payload 정렬을 독립 단계로, 현 단계 공통 `JobResult` 유지 | **PARTIAL** | 결정 Accepted(`adr/0019` 2026-06-14). 현 단계 계약=공통 `JobResult`(데모 §5.2 구조) 확정 / 평면 목표 정렬=후속 Track 잔여(`occurred_at`=T3-7 등). T4-2 토픽 분리 + 후속 Track |
...
| T4-5 | envelope/topic contract 문서 갱신 | `통합본 §8.3 ADR#2`, `통합본 §8.3 ADR#5`, `통합본 §8.3 ADR#6`, `docs/kafka-payloads.md`, `docs/envelope.md` | monitoring-meta | IN_PROGRESS | repo 구현 일관성 | T0/T4 decisions | `handoff/phase1-044/phase1-044-contract-doc-update.md` | §7 8토픽 계약 매트릭스 갱신 + drift check. **1차 정합화 DONE(2026-06-13, phase1-044)**: 계약 3문서(kafka-payloads·envelope·통합본 §6.9/§8.3)+§7 매트릭스 정합 재확인(8토픽 계약 내용 변경 0), stale 구분할파일 참조 5곳 정정(kafka-payloads 2·envelope 3 → master-design.md/13장 표기), spec-drift 보고. **2차 갱신 대기(→ 완료 시 DONE)**: T4-2 분리 구현 후 meta 복귀해 현행 물리명(`job-results`→분리) 반영 |
