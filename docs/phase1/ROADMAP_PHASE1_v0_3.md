...
| #5 | 토픽 명명 | 의미 기반 규칙 B(단일 command-topic, zone suffix=다중 zone 진입 시) | **PARTIAL** | 토픽 *명명 규칙*(D-4(1) — 2026-06-06 RESOLVED, 후보 B, `adr/0005` Accepted) + 토픽 재구조 *실행 순서*(D-4(2) — 2026-06-04 RESOLVED, envelope 먼저). **토픽 재명명(T4-1) 완료(2026-06-07, 3토픽 물리=논리, e2e 58/0/0)**. 토픽 재명명(T4-1)·분리(T4-2 2026-06-14 완료, e2e 64/0/0). 잔여=alert/notification 토픽 신설(Phase 1 확정, §6.9.3·§6.9.5) |
