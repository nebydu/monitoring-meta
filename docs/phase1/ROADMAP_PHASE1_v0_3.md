...
> **📌 현재 액티브 큐** (§17 D-목록·§16/§9~§14의 **사본** — 갱신 2026-06-25 · 표기는 항상 **이름(ID)** 꼴, ID 읽는 법: `docs/phase1/ID-GLOSSARY.md`)
>
> **진행 중 — 형제 repo 발주됨**
> - **alert/notification 토픽 추가 + 신규 토픽 키 정의**(T2-4 ∪ T4-4 합본, work-id `topics-and-keys`, 발주 2026-06-25) — hub(토픽 상수 2개 + producer 골격, 작업 깊이 (b))·infra(kafka-init 5→7종 = 8토픽 계약 중 Phase 1 활성분, metrics=Phase 2 미생성) 발주. 키 wire 인코딩 `alert-key:v1:{rule_id}:{target_id}`(null이면 `agent-offline`)·notification=`incident_id`는 **kafka-payloads·통합본 §6.8.2에 반영(규범 출처=spec)**, x-source는 envelope §2.4 필수 헤더 참조(placeholder 금지=구현 규칙). proposal-review approve. 복귀 게이트=e2e 회귀+계약 일관성. script-agent 영향 0. → §11/§13, `handoff/topics-and-keys/`
>
> **임계 경로 후속**
...
>
> **최근 완료**
> - **result-topic 분리 + 계약 문서 갱신**(T4-2/T4-5, 2026-06-14, Track 4) — `job-results` → `result-topic-job`/`result-topic-log` 분리: 형제 repo 3곳 구현(hub 멀티토픽 구독·script-agent env 2키 분기 발행·infra 2토픽+구토픽 제거) + **동적 e2e 64/0/0**(`e2e/results/20260614-164044.md`: 신토픽 5종·§6-LOG result-topic-log 흐름·구 job-results 0). 계약 문서 5종 갱신(kafka-payloads 매핑표·물리명·통합본 §6.9.5/§6.9.2/§4.4.1·ROADMAP). **payload 위상=ADR#19 신설**(현 단계 공통 JobResult 유지, 목표 평면 정렬은 후속 Track 미배정). `handoff/phase1-041/`·`handoff/phase1-044/`. → §13
...
| #5 | 토픽 명명 | 의미 기반 규칙 B(단일 command-topic, zone suffix=다중 zone 진입 시) | **PARTIAL** | 토픽 *명명 규칙*(D-4(1) — 2026-06-06 RESOLVED, 후보 B, `adr/0005` Accepted) + 토픽 재구조 *실행 순서*(D-4(2) — 2026-06-04 RESOLVED, envelope 먼저). **토픽 재명명(T4-1) 완료(2026-06-07, 3토픽 물리=논리, e2e 58/0/0)** + **토픽 분리(T4-2) 완료(2026-06-14, e2e 64/0/0)**. 잔여=alert/notification 토픽 신설(Phase 1 확정, §6.9.3·§6.9.5) |
...
| #19 | result payload 정렬 단계화 | 토픽 분리(T4-2)와 payload 정렬을 독립 단계로, 현 단계 공통 `JobResult` 유지 | **PARTIAL** | 결정 Accepted(`adr/0019` 2026-06-14). 현 단계 계약=공통 `JobResult`(데모 §5.2 구조) 확정·**T4-2 토픽 분리 완료**(2026-06-14). 평면 목표 정렬=후속 Track 잔여: `occurred_at`=T3-7/ADR#10, `file_state`=ADR#14(NO-OP, T5-4), **status enum·`mode`·`metrics`·`stdout_ref`·중첩→평면·job_type화 = 전담 Track 미배정(후속 배정 필요)** |
...
| T2-4 | `alert-topic` / `notification-topic` 추가 | `통합본 §6.9(다)`/§6.9.3, `통합본 §6.9.5`, `통합본 §8.3 ADR#5`, `envelope §4.1` | hub, infra, monitoring-meta | **IN_PROGRESS (발주 2026-06-25)** | Alert → Notification pipeline | T1-1 등 선행 인프라(=서비스 의존; 토픽/키 계약은 선제 확정 가능) | `handoff/topics-and-keys/` (**T4-4와 합본**, 000-impact + hub + infra) | topic contract, producer/consumer test. 비고: 토픽 신설=Phase 1 확정(§6.9.3·§6.9.5), key rule(alert=`(rule_id,target_id)`/notification=`incident_id`) 확정, 최종 명명(`alert-topic`/`notification-topic`, 규칙 B)=D-4(1) 2026-06-06 승인 확정. **발주 범위=hub(토픽 상수 2개+producer 골격, 작업 깊이 (b))·infra(kafka-init 사전생성 5→7종 = §7 8토픽 논리 계약 중 Phase 1 런타임 활성분; `metrics-topic`은 Phase 2라 미생성). script-agent 영향 0(producer·consumer 전부 hub/BE 측, §4.4.1). proposal-review approve.** |
...
| ID | 항목 | source_ref | owner_repo | status | blocks | blocked_by | handoff | acceptance_evidence |
|---|---|---|---|---|---|---|---|---|
| T4-1 | 토픽 명명: zone 단위 + 의미 기반 (규칙 B 재명명 구현) | `통합본 §8.3 ADR#5`, `통합본 §4.4.1`, `adr/0005-topic-naming.md` | hub, script-agent, infra, monitoring-meta | **DONE (2026-06-07, e2e PASS 58/0/0)** | producer/consumer 전체 | 3토픽 재명명 완료(`commands`→`command-topic`/`audit-events`→`audit-topic`/`heartbeats`→`heartbeats-topic`), `job-results`는 T4-2. infra·hub `KafkaConfig.Topics`·script-agent `config.go` 적용(구현 증거=아래 e2e 결과). 결정 6건(동시 컷오버/클린 재기동/env키 유지/회귀0=R-A+R-B/baseline 재생성/문서 동기화) 반영 | `handoff/phase1-040/phase1-040-000-impact.md` + `handoff/phase1-040/phase1-040-{infra,hub,script-agent}.md` | **e2e 종단 검증 PASS 58/0/0**(`e2e/results/20260607-080703.md`, 하네스 v9): R-B 재명명 완전성(신명 3종/구명 0) + R-A 동작 등가(command-topic 발행 라이브 `COMMAND sent offset=0`, audit-topic AGENT_STARTED 수신, heartbeats-topic 라이브, envelope 독립) + 정적·단위(mvn90/go) 회귀 0. 직전 reuse-infra race(coordinator readiness)는 클린 부팅서 미재현 — 재명명 무관 확정 |
| T4-2 | `job-results` → `result-topic-job/log` 분리 | `통합본 §6.9(나)`/§6.9.2 항목1, `통합본 §8.3 ADR#5` (ADR#5 간접 소속, D-5 RESOLVED) | hub, script-agent, infra | **DONE (2026-06-14, e2e 64/0/0)** | result pipeline | T4-1(DONE)·D-4(1)·D-4(2)·D-5 모두 RESOLVED. 분리 자체=Phase 1 확정(§6.9.2 항목1) | `handoff/phase1-041/` (000-impact + infra/hub/script-agent) | job/log result e2e(R-A+R-B+R-C). **payload 위상 소유 = ADR#19**(Accepted 2026-06-14, `adr/0019`) — 2026-06-13 handoff §5 결정(payload 경계=토픽만 분리, 현 단계 공통 JobResult 유지, 평면 목표 정렬은 후속 Track)은 ADR 승격 전 입력. / 동시 컷오버+구 토픽 제거 / env 2키 신설 / 회귀=동작등가+완전성+분기정확성. **구현 완료**(hub `RESULT_JOB`/`RESULT_LOG` 멀티토픽 구독·script-agent env 2키 job_type 분기 발행·infra 2토픽 생성+구 `job-results` 제거) + **동적 e2e 64/0/0**(`e2e/results/20260614-164044.md`: 라이브 신토픽 5종·§6-LOG result-topic-log 흐름 실증·구 job-results 0) + 계약 문서 5종 갱신 완료(meta 복귀 게이트). payload는 ADR#19로 현 단계 공통 JobResult 유지(목표 평면 정렬은 후속 Track) |
...
| T4-4 | 메시지 키 토픽별 정의 (ADR#6 잔여: 신규 토픽 key) | `통합본 §8.3 ADR#6`, `통합본 6.8.2`, `통합본 §6.9.5`, `docs/kafka-payloads.md` | hub, monitoring-meta (script-agent·infra: 아래 비고) | **IN_PROGRESS (발주 2026-06-25)** | ordering/partitioning semantics | topic naming decision(T4-1, DONE) | `handoff/topics-and-keys/` (**T2-4와 합본**, 000-impact + hub + infra) | topic별 key rule + test (기존 토픽 키 기구현 — §5.1 #6 / 신규: alert=`(rule_id,target_id)`·notification=`incident_id`는 §6.9.5 확정). **규범 출처=spec**: alert key wire 인코딩 `alert-key:v1:{rule_id}:{target_id}`(null이면 `agent-offline`)은 `docs/kafka-payloads.md` `alert-topic` 키 절 + 통합본 §6.8.2에 반영됨(meta 결정 2026-06-25) — 본 행은 그 참조다. `x-source`는 envelope §2.4상 **이미 필수 헤더**이며 "placeholder 금지·publisher 주입"은 그 필수 요건의 구현 규칙(envelope 신규 제약 아님). proposal-review approve. **owner_repo 주**: 신규 토픽 키 적용은 hub, 계약은 meta. 합본의 infra는 T2-4(토픽 생성)분이고, script-agent는 신규 토픽 키 분 영향 0(기존 result 키 `agent_id` 불변). |
| T4-5 | envelope/topic contract 문서 갱신 | `통합본 §8.3 ADR#2`, `통합본 §8.3 ADR#5`, `통합본 §8.3 ADR#6`, `docs/kafka-payloads.md`, `docs/envelope.md` | monitoring-meta | **DONE (2026-06-14)** | repo 구현 일관성 | T0/T4 decisions | `handoff/phase1-044/phase1-044-contract-doc-update.md` | §7 8토픽 계약 매트릭스 갱신 + drift check. **1차 정합화 DONE(2026-06-13)**: stale 구분할파일 참조 5곳 정정. **2차 갱신 완료(2026-06-14)**: T4-2 구현 후 meta 복귀해 kafka-payloads 매핑표·절 물리명·통합본 §6.9.5/§6.9.2/§4.4.1·ROADMAP 일괄 갱신 + ADR#19(payload 위상) 신설 + envelope 참조형화 |
...
> **§16 ↔ Track 표 성격 주 (gate 4차 — spec 6 해소)**: D-9 해소로 성격이 올라간 토픽 handoff 두 건(T2-4 → 현 `handoff/topics-and-keys/`(**T2-4 ∪ T4-4 합본** 발주 2026-06-25 — 구 계획 경로 `phase1-023`(T2-4)·`phase1-043`(T4-4) 둘 다 이 합본으로 대체), T4-2 → `phase1-041-result-topic-split`)을 아래 우선 슬라이스 목록에 **추가**한다. ADR#5(`adr/0005-topic-naming.md`)는 D-4(1) 구체 명명 규칙 승인용 신규 ADR로, 2026-06-06 후보 B로 승인 완료(Accepted)되어 T4-1(`phase1-040-topic-naming`) 재명명 handoff의 선행 결정이 닫혔다. 그 밖의 모든 토픽/도메인 handoff는 §16이 아니라 **Track 표(§9~§14)에서 전체 추적**되며, §16은 대표 슬라이스만 담는다. 따라서 "§16에 없는 handoff = 누락"이 아니라 "Track 표에서 추적 중"이다(Track 표 ↔ §16 성격 충돌 해소).
...
| 9 | `handoff/topics-and-keys/` (발주 완료 2026-06-25) | **T2-4 ∪ T4-4 합본** — `alert-topic`/`notification-topic` 추가 + 신규 토픽 메시지 키 정의(alert wire `alert-key:v1:...`/notification=`incident_id`). hub 작업 깊이 (b)·x-source 필수 주입. proposal-review approve. (계획상 분리 경로였던 phase1-023/phase1-043은 합본으로 대체) | hub, infra (script-agent 영향 0), monitoring-meta(복귀 게이트) |
