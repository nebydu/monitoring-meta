...
>
> **지금 가능**
> - [작업] **T4-1** 토픽 재명명 구현(규칙 B, 3토픽 `commands`/`audit-events`/`heartbeats` → `*-topic`) — **handoff 분배 완료**(2026-06-06, `handoff/phase1-040-000-impact.md` + `phase1-040-{infra,hub,script-agent}.md`). 결정 6건 확정(동시 컷오버/클린 재기동/env키 유지/회귀0=R-A+R-B/baseline 재생성/문서 동기화). **repo 실행 대기**(infra→hub+script-agent 동시→meta e2e §3.3). `job-results` 분리는 T4-2(D-5). → §13
...
| ID | 항목 | source_ref | owner_repo | status | blocks | blocked_by | handoff | acceptance_evidence |
|---|---|---|---|---|---|---|---|---|
| T4-1 | 토픽 명명: zone 단위 + 의미 기반 (규칙 B 재명명 구현) | `통합본 §8.3 ADR#5`, `통합본 §4.4.1`, `adr/0005-topic-naming.md` | hub, script-agent, infra, monitoring-meta | **TODO (handoff 분배 완료 2026-06-06, repo 실행 대기)** | producer/consumer 전체 | (해소 — `adr/0005` Accepted 2026-06-06, D-4(1) RESOLVED). 실행 순서는 D-4(2) RESOLVED(envelope 먼저) — Track 0 이후 착수. 영향 분석·핸드오프 분배 완료(아래 handoff). 범위=3토픽(`commands`→`command-topic`/`audit-events`→`audit-topic`/`heartbeats`→`heartbeats-topic`), `job-results`는 T4-2. 결정 6건 확정(동시 컷오버/클린 재기동/env키 유지/회귀0=R-A+R-B/baseline 재생성/문서 동기화 포함) | `handoff/phase1-040-000-impact.md`(분석) + `handoff/phase1-040-{infra,hub,script-agent}.md`(분배) | topic contract matrix(§7 연계) + 현행 물리명→최종 논리명 재명명 + 회귀 검증(Phase 0 회귀 0=동작 등가 R-A + 완전성 R-B). 실행 순서 infra→hub+script-agent 동시→meta e2e(§3.3). 비고: 명명 *원칙*·*구체 규칙(B)* 모두 확정(`adr/0005` Accepted), 실제 재명명 구현만 잔여 |
