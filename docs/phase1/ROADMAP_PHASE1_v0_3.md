...
>
> **지금 가능**
> - [작업] **T4-2** `job-results` → `result-topic-job`/`result-topic-log` 분리 — 분리 자체=Phase 1 확정(§6.9.2 항목1), 최종 명명 규칙=D-4(1) RESOLVED. **단 ADR 소속이 D-5(미결)** — 같이 풀어야 착수. → §9 Track 4
> - [결정] **D-3** 영속(#12)·인증(#7) 실행순서 / **D-5** 데모 정정 ADR 소속 3건(T4-2 선행). → §17
>
> **막힘 (외부/선행 대기)**
> - **D-1·D-2·D-8** — 통합본 Open(§J/§C/§A). 외부 정보 필요 — 추측 금지.
> - **§16-5**(모듈 분리)=D-2 후 / **D-6**(owner_repo)=D-2 후.
>
> **최근 완료**
