...
> **적용 범위**: ROADMAP 액티브 큐(전환 완료) + **이후 새로 쓰거나 고치는 모든 문장**. ROADMAP 본문에 남은 기존 ID-first 표기(예: "**T4-2 정정**" 같은 주석 제목)는 소급 일괄 치환하지 않고, 해당 부분을 다른 사유로 수정할 때 함께 정리한다(2026-06-11 범위 결정 — 전체 sweep 안 함).
...
> **handoff work-id은 T/D/ADR과 별개 축이다.** `handoff/<work-id>/` 디렉터리명(예: `phase1-041`, `topics-and-keys`)은 *작업 단위* 식별자이지 위 운영 ID 패밀리가 아니다(규약: `handoff/README.md`). **한 work-id가 여러 T를 묶을 수 있다** — 예: `topics-and-keys` = alert/notification 토픽 추가(T2-4) ∪ 신규 토픽 키 정의(T4-4) 합본 발주(2026-06-25). 따라서 ROADMAP의 한 T 행이 가리키는 handoff 경로가 다른 T 행과 같을 수 있다(합본). work-id는 숫자형(`phase1-NNN`)·서술형(`topics-and-keys`) 둘 다 허용된다.
...
- 2026-06-25 — §1에 handoff work-id 축 안내 추가(T와 별개, 한 work-id가 여러 T 묶음 가능 — 예 `topics-and-keys`=T2-4 ∪ T4-4). topics-and-keys 합본 발주 반영.
