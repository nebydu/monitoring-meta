...
- **`HeartbeatConsumer.java` envelope 예외 위상 주석**: e2e 결과(§3 INFO)에서 "envelope 예외 위상 주석이 명시적이지 않음 — 기능 위반은 아님"으로 기록됨. 코드 동작은 정상이나 주석 보강 여부는 hub 세션 결정 사항.
- **`heartbeat.go` 패키지 주석 drift**: 패키지 주석 4~8행에 `otlp_json`·Phase 0 baseline 언급이 잔존(e2e §5 INFO). 코드 동작에는 영향 없으나 ADR-0002 완료 후 구식 주석. 수정 여부는 script-agent 세션 결정 사항.
