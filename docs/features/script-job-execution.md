...
| 8 | `script-agent:internal/jobresult.Publisher` | `jobresult.Publisher.Publish` (job_type=SCRIPT_JOB 분기) | `result-topic-job` | envelope 헤더 **4종 규약**(x-message-id/x-message-version/x-source="script-agent"/x-trace-id — trace 부재 시 생략, command hop과 동일 패턴 `BuildHeaders(id,"")`); key=agentID; env `KAFKA_TOPIC_RESULT_JOB` |
