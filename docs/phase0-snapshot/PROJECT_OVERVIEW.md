...
> **[phase0-snapshot 성격 주석 — 2026-06-05 추가]** 본 문서는 **Phase 0 데모 스냅샷**(참조 전용)이다.
> 이 디렉터리(`monitoring-meta/docs/phase0-snapshot/`)는 데모 spec v0.2.1 단일 기준 문서
> (`monitoring-demo-message-spec-v0.2.1.md`)과 본 개요를 함께 보관한다. 데모 spec 기준 문서는
> 더 이상 `hub/docs/` 또는 `script-agent/docs/`가 아니라 **본 디렉터리**에 있다(각 repo 사본은
> 삭제 예정). 본 문서 내 `hub/docs/...v0.2.1.md` 식 경로 언급은 새 위치
> `monitoring-meta/docs/phase0-snapshot/monitoring-demo-message-spec-v0.2.1.md`로 정정되었다.
> **Phase 1+ 도달 목표 / phase 소속의 기준 문서는 `통합본_v0_9.md` + `docs/phase1/ROADMAP_PHASE1_v0_3.md`다.**
> 본 문서의 phase 소속 서술 중 기준 문서와 충돌하는 부분에는 `superseded by` 표시를 달았다(본문은 보존).
...
> 해당 절도 함께 갱신한다 (ground truth는 `monitoring-meta/docs/phase0-snapshot/monitoring-demo-message-spec-v0.2.1.md` — phase0-cleanup으로 각 repo 사본에서 단일 기준 문서로 이관). **단 이 "코드 변경 시 함께 갱신" 문구는 Phase 0 당시 운영 규칙이며, 본 문서는 Phase 0 snapshot이라 더 이상 갱신 대상이 아니다(상단 성격 주석 참조).**
...
| 성격 | **데모(개발 v0) 단계**. 본개발 reference implementation을 의도 |
...
│   ├── pom.xml
│   ├── docs/
│   │   └── monitoring-demo-message-spec-v0.2.1.md   # 데모 spec 사본(phase0-cleanup으로 삭제 예정; 기준 문서=monitoring-meta/docs/phase0-snapshot/)
...
> **[superseded by 통합본 v0.9 / ROADMAP_PHASE1_v0_3 — 2026-06-05]** 아래 Phase 1/2 구분은
> Phase 0 데모 시점 서술이며 **phase 소속의 기준 문서가 아니다.** 특히 "본개발 Phase 2: …
> Alert / Incident"는 ROADMAP_PHASE1_v0_3 §3·§5.1·§14와 충돌한다. 기준 문서는 **Rule Engine /
> Alert / Incident / Notification(전용 토픽 `alert-topic`/`notification-topic` 신설 포함)을
> Phase 1 확정**으로 본다(통합본 §6.9.3 (다) Phase 1 표 + §6.9.5 토픽표, ROADMAP §3 / §5.1 #18 /
> Track 2). 아래 §8.1 표의 "Alert / Incident 도메인 = Phase 1"과도 본 §3.1이 자체 모순이다.
> phase 소속이 필요하면 통합본 v0.9 / ROADMAP_PHASE1_v0_3을 따른다(아래 본문은 데모 시점 기록으로 보존).

- **데모 단계**: Schema Registry 미도입, 인증 없음, 영속 저장소 없음, in-memory.
  단 코드는 "한 번 시연하고 버리는 demo"가 아니라 **본개발 reference implementation**으로
  유지되며 spec v0.2.1과 일관성을 유지한다.
...
spec v0.2.1의 데모 범위(§0 성격 / §1~§5 토픽·도메인·페이로드 / §4.2 단일
...
> 원본: `monitoring-meta/docs/phase0-snapshot/monitoring-demo-message-spec-v0.2.1.md` (phase0-cleanup으로 각 repo 사본 `hub/docs/`·`script-agent/docs/`에서 단일 기준 문서로 이관; 사본은 삭제 예정).
...
> **[superseded by 통합본 v0.9 / ROADMAP_PHASE1_v0_3 — 2026-06-05]** 아래 phase 소속(특히
> "Alert / Incident 도메인 = Phase 1")은 §3.1의 "본개발 Phase 2: … Alert / Incident"와
> 자체 모순이다. phase 소속의 기준 문서는 통합본 v0.9 / ROADMAP_PHASE1_v0_3이며, 기준 문서는
> **Rule Engine / Alert / Incident / Notification = Phase 1 확정**으로 본다(ROADMAP §3 / §5.1 / Track 2).
> 아래 표는 데모 시점 기록으로 보존한다.
...
- ADR/spec 정밀 검토라면: `monitoring-meta/docs/phase0-snapshot/monitoring-demo-message-spec-v0.2.1.md` (데모 spec 단일 기준 문서)
- 최신 진행 상태: 멀티레포이므로 **저장소별로** `git log --oneline -20` (hub / script-agent / infra 각각)

> **이 문서가 코드와 어긋나는 경우 원본 spec(`monitoring-meta/docs/phase0-snapshot/monitoring-demo-message-spec-v0.2.1.md`)과 실제 코드가 우선**한다. 본 문서는 합본·요약 + 외부 컨텍스트용이며, 운영 단일 진실(single source of truth)는 spec 문서 + 코드.
