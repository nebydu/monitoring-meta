...
이 파일은 **본 시스템 8 Kafka 토픽의 메시지 본문(payload) 구조**. envelope(헤더 메타데이터)는 별도 정본 `docs/envelope.md`가 정의한다(상위 근거: 통합본 6.8 = `v0_9/04_데이터흐름.md` 6.8). 구현 분배(shared-libs/envelope)는 `docs/envelope.md` §7 참조.
