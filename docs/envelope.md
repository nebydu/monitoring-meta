...
> **토픽 명칭 규약**: 위 표의 토픽명은 **논리명**이며, 논리명↔현행 물리명 매핑과 명명 규칙(ADR#5 / `adr/0005-topic-naming.md` — D-4(1) 2026-06-06 Accepted, 후보 B)은 `docs/kafka-payloads.md` "토픽 명명 규칙" 절을 기준 문서로 따른다. `command-topic`은 **suffix 없는 단일 물리 토픽으로 확정**됐다(D-4(1) (2) 승인; 현 단계 zone=1 단일 폐쇄망). **다중 zone 진입 시** `command-topic-{zone}` 전개는 미래 트리거이며(통합본 6.8 "Phase 1 (다중 zone 진입 시)" 조건 + §13_open §A zone topology 해소 후), 지금 고정/추측하지 않는다. **envelope 4종 적용은 토픽명·zone suffix 전개 여부와 무관하게 동일**하다.
>
