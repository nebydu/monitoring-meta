...
### 3.1 원칙 (산문)
envelope은 **도메인 데이터가 아닌 전송 메타데이터**만 담는다. 메시지가 "무엇을 의미하는가"(도메인)는 payload에, "어떻게 전송·식별·추적되는가"(transport)는 envelope 헤더에 둔다. envelope은 **위 §2의 4종 헤더로 한정**되며, 발행자가 임의로 헤더를 늘리지 않는다.
...
| `alert-topic` | 4종 | 표준 적용. payload에 `trace_id` 동일 사본 존재(§5). 키 = `(rule_id, target_id)` 조합 |
...
**불변식 (규범)**:
> payload `trace_id`가 존재할 때, 그 값은 동일 메시지의 헤더 `x-trace-id` 값과 **항상 동일해야 한다**. 둘이 다른 값을 가지는 것은 허용되지 않는다(통합본 6.8.3 "envelope `x-trace-id`와 동일").

- 헤더 `x-trace-id`가 생략된 메시지에서 payload `trace_id`는 부재(optional)이거나, 존재한다면 발행자는 헤더도 동일 값으로 채워야 한다.
...
- **Phase 1 동기화 방식 = 수동 동기화 + spec-sync 통제.** 각 repo가 본 정본을 보고 구현하며, 정본↔구현 drift는 `spec-sync` 검사로 검출한다.
