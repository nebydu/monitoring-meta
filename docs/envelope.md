...
> **문서 성격**
> - 이 문서는 **envelope 4종 헤더 세부 규약의 기준 문서(Phase 1+ 도달 목표)** 이다. 즉 envelope 헤더 규약에 한해 기준 문서가며, **상위 아키텍처 결정은 통합본 v0.9를 따른다**(아래 우선순위 참조).
...
- **envelope 헤더 세부 규약의 기준 문서 = 본 문서(`docs/envelope.md`).** 단 문서 위계상 **통합본 v0.9 6.8/6.9가 상위 근거**이며(상단 문서 성격 참조), 본 문서는 그 아래에서 envelope 4종 헤더의 세부 규약을 정의하는 기준 문서다. 통합본과 충돌 시 통합본이 우선한다.
