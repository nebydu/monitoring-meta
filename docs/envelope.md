# Kafka Envelope 헤더 규약 (envelope 세부 규약 기준 문서)

> **문서 성격**
> - 이 문서는 **envelope 4종 헤더 세부 규약의 기준 문서(Phase 1+ 도달 목표)** 이다. 즉 envelope 헤더 규약에 한해 기준 문서가며, **상위 아키텍처 결정은 통합본(`docs/master-design.md`)을 따른다**(아래 우선순위 참조).
> - 데모 spec v0.2.1 (`docs/phase0-snapshot/monitoring-demo-message-spec-v0.2.1.md` §2.2)은 **Phase 0 코드가 회귀 없이 지켜야 할 동작 spec(ground truth)** 으로 성격이 다르다. 둘을 같은 ground truth로 다루지 않는다.
> - 충돌 시 **통합본 6.8/6.9가 상위 근거**다. 이 문서의 모든 "결정"은 통합본 6.8/6.9 또는 사람이 확정한 Q1~Q5에 근거가 있다.
>
> - 작성 일시: 2026-05-27
> - 근거 문서:
>   - `docs/master-design.md`(통합본) 6.8(메시지 envelope 및 ID 컨벤션) / 6.9(데모 일관성 매트릭스) / 4.4.1(토픽 표) / 8.2(ADR 결정표)
...
- **선택(○)**: 값이 빈 문자열/없음이면 **헤더 자체를 생략**한다(양쪽 repo 구현 일치, 데모 §2.2 "○").
...
> **토픽 명칭 규약**: 위 표의 토픽명은 **논리명**이며, 논리명↔현행 물리명 매핑과 명명 규칙(ADR#5 / `adr/0005-topic-naming.md` — D-4(1) 2026-06-06 Accepted, 후보 B)은 `docs/kafka-payloads.md` "토픽 명명 규칙" 절을 기준 문서로 따른다. `command-topic`은 **suffix 없는 단일 물리 토픽으로 확정**됐다(D-4(1) (2) 승인; 현 단계 zone=1 단일 폐쇄망). **다중 zone 진입 시** `command-topic-{zone}` 전개는 미래 트리거이며(통합본 6.8 "Phase 1 (다중 zone 진입 시)" 조건 + 통합본 13장 §A zone topology 해소 후), 지금 고정/추측하지 않는다. **envelope 4종 적용은 토픽명·zone suffix 전개 여부와 무관하게 동일**하다.
>
> **메시지 키 정책의 기준 문서**은 `docs/kafka-payloads.md` / 통합본 6.8.2이다. 위 표의 키 표기는 envelope 적용 맥락을 위한 **참고**이며, 세부(예: alert-topic의 rule_id null 대체키)는 기준 문서를 따른다.
...
- **payload 직렬화·Phase 성격은 두 토픽이 다르다**(envelope 4종 헤더와는 무관한 별개 사항):
...
- **헤더 `x-trace-id`** — OTel trace context propagation 용. **transport 기준 문서**.
...
**회귀 0 보장 조건**: 헤더 키 4종 문자열 · `x-message-version` 값 `"1"` · source 데모 3종 · 옵셔널 trace 생략 로직. 양쪽 repo 현 구현이 이미 충족한다(1라운드 분석 §1 코드 일치).
...
- **envelope 헤더 세부 규약의 기준 문서 = 본 문서(`docs/envelope.md`).** 단 문서 위계상 **통합본(`docs/master-design.md`) 6.8/6.9가 상위 근거**이며(상단 문서 성격 참조), 본 문서는 그 아래에서 envelope 4종 헤더의 세부 규약을 정의하는 기준 문서다. 통합본과 충돌 시 통합본이 우선한다.
- hub(Java)·script-agent(Go)는 본 문서를 따르는 **언어별 thin 구현 가이드**를 적용한다(언어별 얇은 헤더 빌드/파싱 구현).
- **Phase 1 동기화 방식 = 수동 동기화 + spec-sync 통제**(Q5 확정). 각 repo가 본 기준 문서를 보고 구현하며, 기준 문서↔구현 drift는 `spec-sync` 검사로 검출한다. 단 spec-sync의 **검사 범위·실행 주체·실패 기준 등 운영 세부는 구현 계획으로 별도 정의**하며, 본 문서는 "수동 동기화 + spec-sync 통제"라는 **방식 결정만** 담는다.
...
다음은 결정이 아니라 보존 항목이다. **본 문서 본문에 결정으로 박지 않는다.** Phase에 따라 결정 예정이며 통합본 6.8.6 / 13장(Open)을 참조한다.

| # | 항목 | 출처 | 상태 |
|---|---|---|---|
| O1 | `x-message-id` 중복 검사 **정확한 시점** | 통합본 6.8.6, ADR #15 | Open. baseline(Phase 1 Redis TTL 5분)은 결정, **정확한 시점**은 Open |
| O2 | Schema Registry 도입과 `x-message-version` **정책 연동** | 통합본 6.8.6, ADR #1, 13장 §E | Open. 1차 미도입은 결정, SR 도입 시점·version 연동은 Open |
