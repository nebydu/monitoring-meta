...
마이그레이션 비용에 직접 영향 (09 §11.8). 구 "5단계 AMS 실무자 피드백" 채널 부재로 **사용자 잠정 확정(pull — 차단 시점별 묶음) + 팀 리뷰 재검증** 방식으로 전환(2026-06-13). 항목별 분류·차단 Track·역전 비용은 `handoff/ams-assumption-baseline/ams-assumption-baseline-000-decision-packet.md` §3~§4, 잠정 확정 기록은 `handoff/decisions/ams-assumption-decisions.md`.
