...
**문서 위계**: 최상위 아키텍처·제품 방향 결정은 **통합본(이 파일)이 기준**이다. 단 아래 표의 주제별 분가 문서가 맡은 영역 — payload·envelope의 **필드 단위 상세 계약**과 ADR의 **결정 기록 전문** — 에 한해서는, 본문 요약과 분가 문서가 어긋나면 해당 분가 문서가 우선한다(본문은 방향·결정 사유 요약을 담는다):
