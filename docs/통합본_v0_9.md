...
**이 파일이 통합본의 단일 기준 문서다.** v0.8에서 도입했던 분할 파일 구조(`01_개요.md` ~ `13_open.md`, `CHANGELOG.md`, `v0_9_diff.md`)는 단일 파일로 합쳐진 뒤 폐기됐고, 0~13장과 변경 이력이 모두 이 파일 안에 있다. 본문에 남은 `04 §6.7` 같은 "숫자 §x.y" 표기의 앞 숫자는 구 분할 파일 번호의 잔재다 — **§x.y(장.절)만 보면 된다**(잔재는 본문을 고치는 문장부터 함께 정리).

상세 spec·결정 기록은 아래 분가 문서로 나뉘어 있다. 해당 주제에서 본문과 분가 문서가 어긋나면 **분가 문서가 우선**이다:
...
1. **5단계 AMS 피드백** — `[AMS 분석 가정 — 검증 필요]` 태그 13장 §J 항목 검증
...
- **AMS 분석 가정**: `[AMS 분석 가정 — 검증 필요]` 태그로 분리, 5단계 피드백 시 검증
...
자료 기반 사실과 추론은 분리. 추론은 [추론], 추측은 [추측], 자료는 평서문, 미해결은 [Open question]. AMS 분석 의존 결정은 `[AMS 분석 가정 — 검증 필요]` 태그. 5단계 피드백 후 갱신 대상.
...
**명명 규칙 확정 (v0.11 backfill — `adr/0005-topic-naming.md` Accepted 2026-06-06)**: `<domain>-topic[-{subtype}][-{zone}]` 의미 기반 규칙(후보 B)이 승인됐고 위 8토픽은 그 규칙의 사례다. `command-topic`은 **단일 물리 토픽으로 확정** — zone 단위 전개(`-{zone}` suffix)는 다중 zone 진입 시의 미래 트리거(D-4(1)-future)로 13장 §A에 Open. `heartbeats-topic`의 복수형 domain은 baseline 호환을 위한 **명시 예외**다(ADR-0005 §2.2.1). 토픽 재명명(T4-1)은 2026-06-07 완료 — 3토픽(`commands`/`audit-events`/`heartbeats` → `*-topic`) 물리명=논리명 일치, `job-results`→`result-topic-job/log` 분리(T4-2)만 잔여. **물리 토픽명·payload 상세 계약은 `docs/kafka-payloads.md`가 기준 문서다.**
...
- Agentless polling 결과의 OTLP 변환·합류 방식 — 단일 토픽 vs 별도 토픽
- 메트릭 label 카디널리티 정책
- VictoriaMetrics 단일 인스턴스 vs cluster 모드 (7장)
...
- `command-topic`의 Agent별 routing 방식 (7장)
- Script Agent ↔ BE 통신 — Kafka 직접 vs gRPC stream (7장)
- gRPC stream 채택 시 BE Command Dispatcher 설계 (7장)
- `__consumer_offsets` 운영 부담 검증 (Kafka 직접 채택 시)
- Validation 임시 store 형태 — Redis 단기 캐시 vs in-memory
...
- Rule Engine stateful vs stateless (7장)
- Incident 그룹핑 알고리즘 baseline (7장)
- Alert 도메인 이중 저장 일관성 (7장)
- WebSocket Gateway 권한 필터링 구현 위치 — Gateway 모듈 단일 vs 도메인 BE별 분산
...
- 통보 정책 평가 엔진 — 단순 규칙 매칭 vs Rule Engine 재활용 (7장)
- 다중 폐쇄망에서 통보 발송 위치 — HQ vs Zone (7장 토폴로지와 연동)
- Knox 조직도 동기화 주기 — 변경 webhook vs polling (사이트별 Knox 어댑터 가능성)
...
- 시스템 자체 관측 별도 인스턴스 vs 외부 도구 위임 (7장)
- 감사 로그 보관 기간 + ILM 정책 (3.4 컴플라이언스, 8장)
- 트레이스 백엔드 선정 — Jaeger / Tempo / Zipkin (8장)
- 시스템 자체 OpenSearch 인스턴스 분리 vs 인덱스 분리만 (7장)
...
- Agent State Service vs 데모 AgentRegistry — Phase 1 확장 방향 (7장)
- `heartbeats-topic` 메시지 키 — 데모는 OTel 기본(분배 불보장), Agent 단위 ordering 필요 여부 (8장)
...
> **기준 문서 anchor (v0.11 backfill)**: envelope 헤더의 확정 계약은 `docs/envelope.md`, 토픽별 payload·물리/논리 토픽명 계약은 `docs/kafka-payloads.md`가 기준 문서다. 이 절은 방향·결정 사유를 담고, 필드 단위 상세가 어긋나면 분가 계약 문서가 우선한다.
...
v0.7 결정과 데모가 어긋남. 본개발 Phase 1 진입 시 데모/spec/코드 갱신 필요.

| 결정 항목 | v0.7 결정 | 데모 현재 | 정정 시점 |
|---|---|---|---|
| `job-results` 토픽 분리 | `result-topic-job` + `result-topic-log` 2개 | `job-results` 단일 | Phase 1 — result-topic 분리(T4-2) 잔여 |
...
| `job-results` | `result-topic-job` + `result-topic-log` | (나) 정정 대상 — **T4-2 잔여** |
...
데모 4개 → v0.7 8개. **토픽 재명명(T4-1)은 2026-06-07 완료**(3토픽 물리명=논리명 일치, e2e 회귀 0 — `adr/0005-topic-naming.md`). Phase 1 잔여는 토픽 분리(`job-results` → 2개, T4-2) + 신규 2개(`alert-topic`, `notification-topic`), Phase 2 신규 1개(`metrics-topic`). 토픽 개수는 고정이 아니며 운영/구현 필요에 따라 추가 가능. 물리 토픽명·payload 계약은 `docs/kafka-payloads.md` 기준.
...
- **AMS 분석 가정 검증 단계**가 마이그레이션 전 결정에 영향 — 5단계 피드백 결과 반영
...
AMS 분석 가정 검증 단계(5단계 피드백) 이후 결정 정확도 ↑. 일부 항목은 가정 변경 시 마이그레이션 방식 갱신.
...
- **AMS 분석 가정 검증 단계가 마이그레이션 비용 직접 영향** — 결재 모델 / Drools 룰 복잡도 / 표준 모니터링 차별화 기능이 검증되어야 ETL 비용 정확. 5단계 피드백 우선 진행.
...
본문 각 절에서 Open question 절을 제거하고 이 파일에 카테고리별 누적. 각 항목은 본문 어느 절에서 식별되었는지 표시.
...
- Agentless polling 결과 합류 — 단일 토픽 vs 별도 토픽 (04 §6.1)
...
## J. `[AMS 분석 가정 — 검증 필요]` (5단계 피드백 시)

AMS 실무자 피드백 필요. 마이그레이션 비용에 직접 영향 (09 §11.8).
