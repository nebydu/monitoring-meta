...
| 파일 | 내용 | 페이지 부담 |
|---|---|---|
| `00_INDEX.md` | 이 문서. 전체 목차 + 검토 동선 + 변경 이력 링크 | 짧음 |
| `01_개요.md` | 0. 문서 개요 / 1. 비전·범위 / 2. 가정·제약 / 3. NFR | 중간 |
| `02_매트릭스.md` | 4. 5종 매트릭스 (기능/사용자/이벤트 등) | 중간 |
| `03_기능분류.md` | 5. 기능 분류 (포함/제외/변형/신규) | 중간 |
| `04_데이터흐름.md` | 6. 데이터 흐름 9개 절 + 데모 정합성 매트릭스 | 큼 |
| `05_아키텍처.md` | 7. 시스템 아키텍처 — 토폴로지/논리/물리/Phase 진화/외부 연계 | 큼 |
| `06_기술스택.md` | 8. 기술 스택 확정 + ADR 18개 매핑 | 중간 |
| `09_보안.md` | 9. 인증·인가·암호화·감사·네트워크 + 사이트별 보안 어댑터 | 중간 |
| `10_운영.md` | 10. 자기 관측·SLO·CI/CD·DR·장애 대응 | 중간 |
| `11_마이그레이션_데이터이관.md` | 11·12. 마이그레이션 전략 + 데이터 이관 + 병행 운영 + 롤백 | 중간 |
| `13_open.md` | 13. Open Questions — 단일 집중 (카테고리별) | 중간 |
| `CHANGELOG.md` | 변경 이력 v0.1 ~ v0.9 | 짧음 |
| `v0_9_diff.md` | v0.8 → v0.9 변경분 요약 (검토용) | 짧음 |
...
- **결정 사항만 빠르게 합의** → 각 파일의 `결정 요약` 박스만. 본문 skip.
- **구현 진입 결정** → `04_데이터흐름` + `05_아키텍처` 본문
- **운영 환경 결정** → `05_아키텍처` 7.3 (물리) + `06_기술스택` + `10_운영`
- **보안 결정** → `09_보안` + 관련 어댑터 (`05_아키텍처` 7.6)
- **마이그레이션 / 이중 운영** → `11_마이그레이션_데이터이관`
- **본개발 ADR 작성 입력** → `06_기술스택` 8.3 ADR 매핑 + 각 절 결정 사유
- **보류 사항** → `13_open` 한 파일
- **이번 버전 변경분만** → `v0_9_diff.md`

**검토 원칙:**

1. 각 절 시작의 `결정 요약` 박스가 결정의 진실원. 본문은 결정 사유·짚을 점·다이어그램.
2. 결정 요약을 보고 의심 있는 절만 본문 진입.
3. Open question은 본문이 아니라 `13_open.md`에 집중.
...
- **데모 ↔ v0.x 정합성** — `04_데이터흐름.md` 6.9 매트릭스에 명시. 본문 결정과 데모가 어긋나면 데모가 정정 대상.
- **개발자가 이 문서만으로 환경/기술/아키텍처 판단 가능해야 함**이 v0.x의 목적.
- **분할 = source of truth, 통합본은 derived artifact**. 외부 공유 / 본개발 ADR 작성 시점에 `cat` 으로 합쳐 통합본 생성 가능. 분할이 작업/검토 효율에 더 유리.

---

## 변경 이력

전체 이력은 `CHANGELOG.md`. 최근 버전 요약:

- **v0.9** (2026-05-24) — 9장 보안 / 10장 운영 / 11+12장 마이그레이션·데이터 이관 신설. 본개발 진입 준비 완료
- **v0.8** (2026-05-24) — 7장 시스템 아키텍처 신설, 8장 기술 스택 + ADR 정렬 신설, 분할 구조 도입, 결정 요약 박스, Open question 단일 집중
- **v0.7** (2026-05-24) — 6장 데이터 흐름 신설 (9개 절), OS 1단계 Win/Linux, 옵션 A 확정, Kafka 토픽 8개

v0.8 → v0.9 변경분 요약은 `v0_9_diff.md`.

---

## 다음 단계 (v0.10 이후)

1. **5단계 AMS 피드백** — `[AMS 분석 가정 — 검증 필요]` 태그 13_open §J 항목 검증
2. **(β) vs (γ) 모듈 분리 협의** — 13_open §C 핵심 사안
3. **사이트별 정보 입수** — 13_open §A 카테고리 (운영 환경 정보)
4. **LEGO 호환 PoC** — 06 §8.4
5. **데모 → Phase 1 마이그레이션 계획** — `11_마이그레이션_데이터이관` + 데모 정정 11개 항목 (04 §6.9)
...
- **NFR**: 가용성 99.x %, 응답 시간 정량 미확정 [Open], 데이터 손실 허용 여부 [Open], 보안 정책 사이트별 [Open]
...
- **운영 환경**: 다중 폐쇄망 (Samsung 사내 HQ + 외부 사이트 Zone들). 토폴로지는 (c) Zone 독립 baseline ([상세](05_아키텍처.md#71))
- **OS 범위**: **Windows / Linux 한정** (v0.7 결정). Solaris/AIX/HP-UX는 추후 확장
- **사내 표준 정합**: Knox 조직도/Mail, 사내 SMS, 메신저, Teams Webhook, BCMS/CMDB (사이트별)

### 1.3 산출물 위상

- v0.x = 본개발 설계서
- 데모(walking skeleton) + 본개발 ADR + v0.x가 본개발 단일 진실원
- 데모 정정은 v0.x 결정에 따라 — `04_데이터흐름.md` 6.9 매트릭스 참조
...
- **K8s 운영 가능**. Zone당 자체 K8s 클러스터 운영 (15~18 노드 baseline, [상세](05_아키텍처.md#73))
...
- 외부 시스템 연계는 어댑터 패턴으로 사이트별 가변성 흡수 ([상세](05_아키텍처.md#76))
...
- **데이터 처리 규모** — Agent 수백~수천 / Zone, Job 수천, 룰 수백, 동시 운영자 수십 [추정 — SMS 트래픽 규모 입수 시 재추산, Open]
- **응답 시간** — UI 조회 2초 이내, 실시간 push 1초 이내 [추정, Open]
- **확장 단위** — Kubernetes 수평 확장 + 메시지 처리 인스턴스 분리

### 3.2 가용성

- **목표** — 99.x % [RTO/RPO 구체 미확정, Open]
- **장애 격리** — Resilience4j Circuit Breaker + Bulkhead (외부 시스템 어댑터 단위)
- **데이터 손실 허용** — 모니터링 메트릭 일시 손실 vs 감사 로그 무손실 — 정책 미확정 [Open]

### 3.3 보안

- **인증** — JWT + OIDC + Knox 어댑터 ([상세](06_기술스택.md))
- **mTLS** — Agent ↔ Kafka, BE 모듈 간 (Service Mesh 미도입 baseline에선 Kafka에 한정)
- **감사 로그** — 사용자 액션·명령 실행·권한 변경 모두 `audit-topic` 추적 ([상세](04_데이터흐름.md))
- **Secret 관리** — K8s Secret + External Secrets Operator + Vault/KMS
- **사내 보안 정책 구체** [Open] — Samsung 사내 정책 항목 확인 필요

### 3.4 컴플라이언스

- **감사 로그 보관** — 산업 표준 1년 이상 (5~7년 사이트별) [Open — 보관 기간 운영팀 결정]
- **개인정보 처리** — Knox 사용자/조직도 동기화 영역 [Open]

### 3.5 운영 관측

- **OTel 표준 적용** — 메트릭 / 트레이스 / 로그
- **자기 모니터링** — Zone 내부 별도 OTel Collector + VictoriaMetrics + Jaeger ([상세](05_아키텍처.md#75))
- **사내 표준 모니터링 카탈로그 정합** — 사이트별 카탈로그 파일 형식 / 갱신 주기 / 충돌 처리 [Open]
...
| KMS / Vault | — | — | ○ | **신규 표준** |
| ITOSS / OASIS / RAPIDANT | — | ● | — | 사이트별 사용 여부 추후 [Open] |
...
- 신규 데이터 모델 컨벤션: 단일 prefix 또는 도메인별 스키마 분리 [Open, 7장]. AMS 이원화 답습 안 함.
...
| SMS 외부 연계 | ITOSS / OASIS / RAPIDANT | Out | (미상) | 사이트별 [Open] |
...
| `command-topic` (zone 단위) | BE Quartz / 정책 발행 | Script Agent (`group.id=agent-N`) | `target_agent_id` (hash partition) | ● | Job 실행 명령 |
...
| `heartbeats-topic` | OTel Collector | Heartbeat Consumer → Redis lastSeen + Agent State Service | OTel 표준 (분배 불보장) | × | Agent heartbeat (OTLP, Phase 1 protobuf 전환) |
...
| Schema 관리 | 없음 | 없음 | 없음 | Schema Registry 검토 [Open, 8장] |
| **저장소** | | | | |
...
- SMS Backend Java 구체 버전 / Spring 사용 여부 / 빌드 [Open].
- Solaris/AIX/HP-UX Agent 지원 부담 (+3~5 MM 불확실성).
- 단순 DB 다중 지원의 실제 운영 부담 [Open, 9장 + 12장].
- LEGO ↔ 신규 FE 호환 PoC 결과 [Open, +4~6 MM].
...
| **로그 수집 정책 템플릿 (LM)** | 제외 | SMS | LM 미사용 종속 |
...
| 이벤트 Class별 정책 | 변형 | SMS | 신규 정책 체계 흡수 검토 [Open] |
...
- **Job**: 등록 시 PG+Object Storage, 결재 webhook 비동기, Validation sandbox 모드, Quartz Clustered, command-topic zone 단위
...
각 절 Open question은 [`13_open.md`](13_open.md)에 단일 집중. 본문엔 결정 + 흐름 + 짚을 점만.
...
다루는 9개 절: 6.1 메트릭, 6.2 Job 실행 결과, 6.3 로그, 6.4 이벤트(Alert/Incident), 6.5 통보, 6.6 감사·관측, 6.7 Heartbeat 및 Agent 상태, 6.8 메시지 envelope 및 ID 컨벤션, 6.9 데모 정합성 매트릭스.
...
데모 정합성: 본 장의 각 절은 데모 spec v0.2.1과 매핑된다. 매핑 매트릭스는 6.9에 정리.
...
- Agentless Polling Service의 결과 합류 방식이 자료 기반이 아닌 추론. SMS에서는 PollingManager가 별도 처리 경로로 들어갔을 가능성이 높지만(SMS 자료 확인 필요), 신규에서 OTLP 합류 형태로 단일화하는 게 표준 정합. 사이트별 polling 주기 차이가 큰 경우 별도 토픽 분리 검토 [Open].
- CSP별 어댑터에서 들어오는 cloud-native 메트릭은 인스턴스 식별자가 많아 카디널리티 폭발 위험. label 정리 정책 필요 [Open].
...
```mermaid
flowchart LR
    CT[(Kafka<br/>command-topic-zone-N<br/>partition key=hash agent_id)] -->|group.id=agent-N| SA[Script Agent<br/>mode: prod 또는 validation]
...
**명령 수신.** Script Agent는 `command-topic`을 consume해 자기 명령을 받는다. **routing 패턴**: 환경/폐쇄망 단위로 토픽을 묶음(zone당 1개), partition key = `hash(agent_id)`, Agent당 단일 `group.id`, 메시지 header의 `target_agent_id` 필터링. AMS의 IP-Port 동적 토픽 안티패턴 답습 안 함.
...
**두 컨슈머 그룹이 같은 토픽을 직접 consume** (v0.7 옵션 2 결정 정합).
...
- OTel Logs 표준 / ELK 중앙 grok 모델과 정합 (Agent를 얇게 두고 중앙에서 처리)
- raw 로그 보존이 자연스러움 (OpenSearch 전문 검색의 본질 기능)
- 룰 변경 hot-reload가 BE 단일 위치
- Agent OS 호환성 부담 최소화 (Windows/Linux 1차 한정과 정합)
...
- **재시도 정책 baseline**: 지수 백오프 + 최대 5회 (SMS/Email/Messenger), Teams Webhook은 단순 재시도 3회. 채널 어댑터 인터페이스가 재시도 정책을 자체 결정 — Notification Service는 발송 요청 한 번만, 결과 통보 받음. baseline 값은 운영팀 검증 [Open].
...
- 트레이스 context propagation: HTTP/gRPC 자동, Kafka 수동 (헤더 처리). `x-trace-id` 데모 적용 정합 — Phase 1에서 BE consume 시 trace context 복원 코드 필요 (ADR #15 트랙).
...
flowchart LR
    ...
    OC -->|Kafka exporter<br/>otlp_json → protobuf Phase 1| HB[(Kafka<br/>heartbeats-topic)]
...
Agent는 OTel SDK로 `agent.heartbeat` Gauge 메트릭(value=1)을 주기 push. 데모 baseline 주기 10초, timeout 30초. Collector가 OTLP 메시지를 `heartbeats-topic`에 재발행. 직렬화 데모 단계 `otlp_json`, **Phase 1에서 protobuf 전환** (ADR #2).
...
- **heartbeat 토픽 분리가 표준 정합.** metrics-topic과 합치는 옵션 있지만 책임 다름 (heartbeat은 상태 판정, metric은 시계열 저장). 별도 토픽 유지. v0.6 4.4.1에 `heartbeats-topic` 추가됨.
...
- **protobuf 전환 incompatibility.** Phase 1 진입 시 OTel Collector exporter encoding `otlp_json` → `otlp_proto` + Heartbeat Consumer 디코더 동시 변경 필요.
...
| `alert-topic` | `(rule_id, target_id)` 조합 | Rule × 대상 단위 — Dedup 키와 정합 |
...
|---|---|---|---|
| `job-results` 토픽 분리 | `result-topic-job` + `result-topic-log` 2개 | `job-results` 단일 | Phase 1 |
| Heartbeat 직렬화 | protobuf (OTLP) | `otlp_json` | Phase 1 (ADR #2) |
...
| `command-topic` routing | zone 단위 + hash partition | 단일 `commands` 토픽 | Phase 1 (다중 zone 진입 시) |
...
| Agent State Service | 6.7 | AgentRegistry → 별도 서비스 승격 |
...
| 데모 토픽 (현재) | v0.7 토픽 (목표) | 정합 분류 |
|---|---|---|
| `commands` | `command-topic` (zone 단위) | (가) 검증 + (나) zone 분리 정정 |
| `job-results` | `result-topic-job` + `result-topic-log` | (나) 정정 대상 |
| `audit-events` | `audit-topic` | (가) 검증 완료 |
| `heartbeats` | `heartbeats-topic` | (가) 검증 완료 (단 protobuf 정정) |
| — | `metrics-topic` | (다) v0.7 신규 (Phase 2) |
| — | `alert-topic` | (다) v0.7 신규 (Phase 1) |
| — | `notification-topic` | (다) v0.7 신규 (Phase 1) |

데모 4개 → v0.7 8개. Phase 1에서 토픽 분리(`job-results` → 2개) + Phase 1 신규 2개(`alert-topic`, `notification-topic`) + Phase 2 신규 1개(`metrics-topic`). 토픽 개수는 고정이 아니며 운영/구현 필요에 따라 추가 가능.
...
- **모듈 분리**: (β) 잠정 baseline — 모듈러 모놀리스 + 메시지 처리 분리. 약 9개 deployment. (γ) MSA는 **협의 필요**
...
Open question은 [`13_open.md`](13_open.md) 단일 집중.
...
| 표준 모니터링 정합 | 사내 단일 환경 | multi-tenant 표준 | multi-tenant 표준 |
| 1차 baseline 적합도 | × (외부 사이트 충돌) | △ (운영 부담 큼) | ✓ |

### 짚을 점

- **1차 운영 사이트 분포** [Open] — Samsung 사내 단독이라면 (a) 단일이 합리적일 수 있음. 정보 입수 후 baseline 재검토 카드 살아있음.
- **사이트별 권한 모델** [Open, 4.1.8] — (b) HQ 통합 시 권한 통합 모델 별도 설계 필요. 9장에서 다룸.
- **데모 위상** — 데모는 Hub BE 단일이라 (a)에 가까움. Phase 1에서 (c)로 진화하려면 Helm Chart + 멀티 zone 배포 파이프라인 정리 필요.
...
- **Kafka broker 외부 노출 사이트별 보안 정책** [Open] — 직접 노출 어려운 사이트는 BFF Kafka Gateway 패턴 필요, 7.5.3 재고 입력
- **MinIO 자체 운영 vs 사이트 storage** [Open] — 사이트 NetApp/EMC/사내 S3 호환 활용 가능성, 어댑터 패턴으로 추상화
- **노드 사이징 검증** [Open] — SMS 트래픽 규모 입수 후 재추산
...
**정정 사항**: Hub BE → 모듈러 모놀리스 + 메시지 처리 분리, in-memory → 영속, `commands` → `command-topic`(zone), `job-results` → `result-topic-job/log` 분리, `audit-events` actor 확장, `heartbeats` → protobuf, Agent 등록 → 사전 토큰.
...
| Agent State Service vs AgentRegistry | **State Service 승격** | 6.7 정합 |
| 다중 폐쇄망 통보 발송 위치 | **Zone 자체 발송** | 7.1 (c) 정합 |
...
- **결재 webhook Ingress가 가장 까다로움** — Egress는 통제 가능, Ingress는 외부 → Zone. HMAC 서명 검증 + IP allowlist + Ingress 노출. 사이트별 보안 정책 [Open]
...
| **Agent 언어** | **Go** | 데모 검증, 단일 바이너리 + Win/Linux 정합, 메모리 작음 | 운영팀 선호 시 Java 재검토 |
| **Frontend** | **LEGO + WebSocket** | 사내 표준 정합 | LEGO 호환 PoC 결과 별도 트랙 |
...
본개발 진입 시 ADR 문서가 v0.x 본문을 참조. 1:1 정합.

| ADR | 주제 | 데모 | v0.x 본개발 | 본문 |
|---|---|---|---|---|
| 1 | 스키마 관리 | 마크다운 수동 | 1차 미도입, Phase 2/3 Apicurio | 8.2 |
| 2 | Heartbeat 마샬링 | `otlp_json` | Phase 1 protobuf | 04 §6.7 |
| 3 | Audit 채널 | Kafka 직행 | 동일 유지 | 04 §6.6 |
| 4 | Consumer group | Agent별 unique group.id | 동일 + zone 단위 토픽 routing | 04 §6.2, §6.8 |
| 5 | 토픽 명명 | 환경 prefix 없음 | zone 단위 + 의미 기반 | 02 §4.4.1, 04 §6.2 |
...
- **사용자 모델**: AMS 단일 테이블 + Knox 동기화 (02 §4.3 정합)
...
- **사이트별 차이**: 어댑터 패턴 + ConfigMap (05 §7.6 정합)

Open question은 [`13_open.md`](13_open.md). 특히 사이트별 보안 정책 입수 항목 다수.
...
산업별 컴플라이언스 요구로 5~7년 [Open]. 사이트별 보관 기간 정책 ConfigMap.
...
| 영역 | 요구 |
|---|---|
| **개인정보** | Knox 동기화 사용자 정보 (이름/이메일/부서) — 사이트별 처리 정책 [Open] |
...
| **데이터 격리** | 다중 폐쇄망 — Zone 간 데이터 이동 없음 (7.1 (c) 정합) |
...
**경계 1 — Ingress**: 외부 사용자 / 외부 시스템 webhook 진입. TLS + JWT 검증 + IP allowlist + Rate limiting. WAF는 사이트별 정책 [Open].
...
**경계 3 — Zone Egress**: 외부 시스템 호출. 사이트별 방화벽 정책으로 허용 IP/포트 제한. **Knox / SMS / Teams / Messenger / 결재 시스템** 각각 사이트 정책 확인 필요 [Open].

### K8s NetworkPolicy

namespace 간 통신 명시적 허용 (05 §7.3 정합):
...
- **자기 관측**: OTel SDK + Collector self + VictoriaMetrics self + Jaeger + OpenSearch system log index (04 §6.6 정합)
...
- **사내 표준 카탈로그 정합**: 어댑터로 사내 표준 메트릭 export 가능 (사이트별)

Open question은 [`13_open.md`](13_open.md). 특히 SLO 정량 / DR 목표 / 백업 보관 기간.
...
각 BE 모듈이 emit하는 표준 메트릭 (OTel Semantic Conventions 정합):
...
| Script Agent | 사이트 운영 정책 — 무중단 패치 메커니즘 [Open, ADR #11 후속] |
...
RTO/RPO baseline은 미확정 [Open, NFR 3.2]. 사이트별 DR 요구에 따라 조정.

### 복구 절차

1. **데이터 노드 장애** — Operator 자동 failover (CloudNativePG replica 승격, OpenSearch shard 재분배). 수동 개입 거의 없음.
...
- Major: 1시간 내 응답
- Minor: 영업일 처리
- 사이트별 운영팀 분리 (각 Zone 독립 운영, 7.1 (c) 정합)

---

## 10.6 사내 표준 모니터링 카탈로그 정합
...
[Open, NFR 3.5] — 사내 표준 카탈로그가 요구하는 메트릭 명세 입수 후 매핑. baseline은 10.1 표준 메트릭 + 도메인 메트릭 일부.

---

## 10.7 짚을 점

- **SLO baseline은 추정치** — 운영 데이터 입수 후 SMS 트래픽 규모 + 사이트 운영 요구 반영해 조정 [Open, NFR 3.1]
- **RTO/RPO baseline 미확정** — 사이트별 DR 요구에 따라 조정. 1시간 RPO는 백업 주기와 직결 [Open, NFR 3.2]
- **Agent 무중단 패치** [Open, ADR #11 후속] — Script Agent가 OS 호스트에 설치되어 패치 시 영향. baseline은 사이트 운영 정책 의존
...
- **Phase 진입과 정합**: 데모(Phase 0) → 본개발 완료(Phase 1) → 첫 사이트 cutover → 점진 확장
- **AMS 분석 가정 검증 단계**가 마이그레이션 전 결정에 영향 — 5단계 피드백 결과 반영

Open question은 [`13_open.md`](13_open.md). 특히 SMS 운영 데이터 규모 / AMS 결재 모델 검증 / 사이트별 cutover 일정.
...
| **룰 (Drools 7.x)** | Rule Engine (Drools 8.x) | **재작성 (7.x → 8.x KIE API 차이)**. v0.7 8장 결정 정합 |
...
| **모니터링 대상** | Agent 등록 (KMC 기반) `[AMS 분석 가정]` | OpenITSM/MSP+ 종속도 검증 후 결정 |
...
| **AI 요약·분석 (F09)** | 향후 (1차 제외) | 1차에 별도 트랙. v0.6 5.4 KDB AI placeholder 정합 |
...
- **변환** — SMS/AMS 도메인 모델 → 신규 도메인 모델 매핑 (v0.x 02 §4.2 정합). prefix 이원화 흡수 (AMS `TB_` vs `ams_` → 단일 prefix).
...
- **데이터 비교** — daily 또는 hourly 정합성 검사 (Alert 수 / Incident 수 / 통보 발송 수)
...
- **데모(Phase 0) → Phase 1 전환 = 본개발의 마이그레이션** — 데모는 별도 환경이므로 사이트 cutover와 무관. 본개발 진입 시 데모 코드 ↔ Phase 1 코드 분리 정합 (04 §6.9 데모 정합성 매트릭스).
...
- 사내 표준 모니터링 메트릭과 신규 메트릭 정합화 (01 §3.5, 08 §10.6)
...
- `__consumer_offsets` 운영 부담 실측 — 7.5.3 (a) Kafka 직접 채택 정합 (05 §7.5)
...
- 데모 코드 패키지 경계가 모듈러 모놀리스 분해와 정합하는지 (05 §7.2.4)
...
- 서브시스템 모델의 OpenITSM/MSP+ 종속도 (02 §4.1.1)
