...
| `04_데이터흐름.md` | 6. 데이터 흐름 9개 절 + 데모 일관성 매트릭스 | 큼 |
...
- **데모 ↔ v0.x 일관성** — `04_데이터흐름.md` 6.9 매트릭스에 명시. 본문 결정과 데모가 어긋나면 데모가 정정 대상.
...
- **사내 표준 모니터링 카탈로그 일치** — 사이트별 카탈로그 파일 형식 / 갱신 주기 / 충돌 처리 [Open]
...
- Agentless Polling Service의 결과 합류 방식이 자료 기반이 아닌 추론. SMS에서는 PollingManager가 별도 처리 경로로 들어갔을 가능성이 높지만(SMS 자료 확인 필요), 신규에서 OTLP 합류 형태로 단일화하는 게 표준 일치. 사이트별 polling 주기 차이가 큰 경우 별도 토픽 분리 검토 [Open].
...
본개발 진입 시 ADR 문서가 v0.x 본문을 참조. 1:1 일치.
