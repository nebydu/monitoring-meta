...
> **버전:** v0.10
> **파일명:** `통합본_v0_9.md` 유지 — 외부 참조(경로·링크)의 안정 앵커라 내부 버전과 별개로 고정. 경위는 변경 이력 v0.10 행 참조.
> **최종 갱신일:** 2026-06-11
...
- **보류 사항** → `13_open` 한 파일
- **v0.9 변경분(v0.8→v0.9)** → `v0_9_diff.md` / **v0.10 변경분(표기 전용)** → 하단 변경 이력 v0.10 행 + `handoff/v0-10-notation/`
...
2. **모듈 분리 협의 — (β) 모듈러 모놀리스+메시지 처리 분리 vs (γ) 풀 MSA** — 13_open §C 핵심 사안
