...
이 repo(`monitoring-meta`)는 **코드를 만들지 않는다.** 정본 spec/문서/종단 검증 산출물만 다루는 오케스트레이터다. 통합본 v0.9 정본을 들고 형제 repo(`../hub`, `../script-agent`)에 작업 spec을 분배하고, polyrepo 종단 검증을 수행한다.
...
- 아직 결정되지 않은 ADR.
- 데모 spec과 통합본의 위상 충돌로 어느 쪽을 따라야 할지 모호한 경우.
...
5. 세션 Stop 시 `codex-gate` hook이 Codex로 정합성 검증.
...
| `spec-sync` | sonnet | Read, Grep, Glob, Write | `handoff/` | 정본↔사본 spec drift 검출(보고만) |
...
## 7. 디렉터리 약속
- `docs/` — 정본 spec·문서 (통합본, kafka-payloads, envelope 등).
- `adr/` — ADR 결정 기록.
...
세션 종료 시 `.claude/hooks/codex-gate.sh`가 자동 실행되어, spec 관련 변경이 있을 때만 Codex로 "통합본 내부 일관성 + 8토픽 spec 정합성 + ADR/Open 정합성"을 read-only 검증한다. fail이면 종료가 막히고 다음 턴에 수정 작업으로 이어진다. 상세는 hook 스크립트 상단 주석 참조.
