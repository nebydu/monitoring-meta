...
- `docs/phase1/ID-GLOSSARY.md` — 식별자 읽는 법 범례. **신규 운영·계획 ID는 T(작업)/D(결정)/ADR(결정 기록) 3종만 생성** — 폐지 패밀리(G, DoD, N/C/S/drift, P/X/M) 신규 생성 금지. **표기는 이름 우선(name-first)**: 문장에서 ID 단독 인용 금지, 항상 `이름(ID)` 꼴(예: "result-topic 분리(T4-2)") (2026-06-11 결정, `handoff/id-cleanup/`).
